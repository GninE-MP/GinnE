...
void Initialize(void)
{
    ...
    if (hGTA != NULL)
    {
        ...
        if (pKernel32)
        {
            ...
            {
                // Tell GninE we successfully hooked the trainer
                PostMessage(hGTA, MESSAGE_MONITOR_START, 0, 0);
                return;
            }
        }

        // Tell GninE we successfully injected but failed hooking the functions
...
void Finalize(void)
{
    ...
    // Tell GninE we've unhooked from the trainer
    if (hGTA != NULL)
    {
