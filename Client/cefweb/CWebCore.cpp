...
bool CWebCore::Initialise()
{
...
#ifndef GninE_DEBUG
    CefString(&settings.browser_subprocess_path).FromWString(FromUTF8(CalcGninESAPath("GninE\\CEF\\CEFLauncher.exe")));
#else
    CefString(&settings.browser_subprocess_path).FromWString(FromUTF8(CalcGninESAPath("GninE\\CEF\\CEFLauncher_d.exe")));
#endif
    CefString(&settings.resources_dir_path).FromWString(FromUTF8(CalcGninESAPath("GninE\\CEF")));
    CefString(&settings.cache_path).FromWString(FromUTF8(CalcGninESAPath("GninE\\CEF\\cache")));
    CefString(&settings.locales_dir_path).FromWString(FromUTF8(CalcGninESAPath("GninE\\CEF\\locales")));
    CefString(&settings.log_file).FromWString(FromUTF8(CalcGninESAPath("GninE\\CEF\\cefdebug.txt")));
#ifdef GninE_DEBUG
    ...
    CefRegisterSchemeHandlerFactory("http", "GninE", app);
...
void CWebCore::InitialiseWhiteAndBlacklist(bool bAddHardcoded, bool bAddDynamic)
{
    if (bAddDynamic)
    {
        ...
        static SString whitelist[] = {"google.com",         "youtube.com", "www.youtube-nocookie.com", "vimeo.com", "player.vimeo.com",
                                      "code.jquery.com",    "GninEsa.com",   "gnine.com",       "GninEvc.com", "www.googleapis.com",
                                      "ajax.googleapis.com"};
...
bool CWebCore::MakeSureXMLNodesExist()
{
    ...
    if (!m_pXmlConfig)
    {
        SString browserDataPath = CalcGninESAPath(GninE_BROWSERDATA_PATH);
