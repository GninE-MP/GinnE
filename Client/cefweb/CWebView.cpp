...
bool CWebView::SetAudioVolume(float fVolume)
{
    ...
    SString strJSCode(
        "function GninE_adjustAudioVol(elem, vol) { elem.volume = vol; elem.onvolumechange = function() { if (Math.abs(elem.volume - vol) >= 0.001) elem.volume "
        "= vol; } }"
        "var tags = document.getElementsByTagName('audio'); for (var i = 0; i<tags.length; ++i) { GninE_adjustAudioVol(tags[i], %f); }"
        "tags = document.getElementsByTagName('video'); for (var i = 0; i<tags.length; ++i) { GninE_adjustAudioVol(tags[i], %f); }",
        fVolume, fVolume);
                                                                      ...
                                                                      CefRefPtr<CefCallback> callback)
{
    ...
    // Add some information to the HTTP header
    {
        ...
        {
            // Add GninE:SA "watermark"
            iter->second = iter->second.ToString() + "; " GninE_CEF_USERAGENT;
