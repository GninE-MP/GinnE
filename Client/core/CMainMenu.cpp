...
#define CORE_GninE_MENUITEMS_START_X  0.168

#define CORE_GninE_BG_MAX_ALPHA       1.00f   //ACHTUNG: Set to 1 for now due to GTA main menu showing through (no delay inserted between Entering game... and loading screen)
#define CORE_GninE_BG_INGAME_ALPHA    0.90f
#define CORE_GninE_FADER              0.05f // 1/20
#define CORE_GninE_FADER_CREDITS      0.01f

#define CORE_GninE_HOVER_SCALE        1.0f
#define CORE_GninE_NORMAL_SCALE       0.6f
#define CORE_GninE_HOVER_ALPHA        1.0f
#define CORE_GninE_NORMAL_ALPHA       0.6f

#define CORE_GninE_HIDDEN_ALPHA       0.0f
#define CORE_GninE_DISABLED_ALPHA     0.4f
#define CORE_GninE_ENABLED_ALPHA      1.0f

#define CORE_GninE_ANIMATION_TIME     200
#define CORE_GninE_MOVE_ANIM_TIME     600

#define CORE_GninE_STATIC_BG          "cgui\\images\\background.png"
#define CORE_GninE_LOGO               "cgui\\images\\background_logo.png"
#define CORE_GninE_FILLER             "cgui\\images\\GninE_filler.png"
#define CORE_GninE_VERSION            "cgui\\images\\version.png"
...
CMainMenu::CMainMenu(CGUI* pManager)
{
    ...
    m_pFiller->LoadFromFile(CORE_GninE_FILLER);
    ...
    m_pBackground->LoadFromFile(CORE_GninE_STATIC_BG);
    ...
    m_pFiller2->LoadFromFile(CORE_GninE_FILLER);
    ...
    m_pLogo->LoadFromFile(CORE_GninE_LOGO);
    ...
    m_pVersion->LoadFromFile(CORE_GninE_VERSION);
    ...
    m_menuAY = m_iFirstItemCentre - fFirstItemSize * (CORE_GninE_HOVER_SCALE / CORE_GninE_NORMAL_SCALE) * 0.5f;            // Top side of the items
    ...
    m_pMenuArea->LoadFromFile(CORE_GninE_FILLER);
    ...
    for (uint i = 0; i < CORE_GninE_NEWS_ITEMS; i++)
    {
        fDrawPosY += 20;
    ...
    #define XP_VISTA_WARNING _("GninE will not receive updates on XP/Vista after July 2019.\n\nUpgrade Windows to play on the latest servers.")
...
void CMainMenu::Update()
{
    ...
    if (m_bIsFullyVisible)
    {
            ...
            (cursor.y < m_menuBY + BODGE_FACTOR_4))
        {
            ...
            if (m_pHoveredItem)
            {
                float fProgress = (m_pHoveredItem->image->GetAlpha() - CORE_GninE_NORMAL_ALPHA) / (CORE_GninE_HOVER_ALPHA - CORE_GninE_NORMAL_ALPHA);
                // Let's work out what the target progress should be by working out the time passed
                fProgress = ((float)ulTimePassed / CORE_GninE_ANIMATION_TIME) * (CORE_GninE_HOVER_ALPHA - CORE_GninE_NORMAL_ALPHA) + fProgress;
        ...
        {
            float fProgress = ((*it)->image->GetAlpha() - CORE_GninE_NORMAL_ALPHA) / (CORE_GninE_HOVER_ALPHA - CORE_GninE_NORMAL_ALPHA);
            // Let's work out what the target progress should be by working out the time passed
            // Min of 0.5 progress fixes occasional graphical glitchekal
            fProgress = fProgress - std::min(0.5f, ((float)ulTimePassed / CORE_GninE_ANIMATION_TIME) * (CORE_GninE_HOVER_ALPHA - CORE_GninE_NORMAL_ALPHA));
    ...
    if (m_iMoveStartPos)
    {
        ...
        float fMoveTime = (fTickDifference / CORE_GninE_MOVE_ANIM_TIME);
        ...
        m_pDisconnect->image->SetAlpha(m_bIsIngame ? fMoveTime * CORE_GninE_NORMAL_ALPHA : (1 - fMoveTime) * CORE_GninE_NORMAL_ALPHA);
        ...
        if (fMoveTime == 1)
        {
            ...
            else
            {
                ...
                m_menuAY = fTopItemCentre - fTopItemSize * (CORE_GninE_HOVER_SCALE / CORE_GninE_NORMAL_SCALE) * 0.5f;            // Top side of the items
                m_menuAY += BODGE_FACTOR_1;
    ...
    if (m_ucFade == FADE_IN)
    {
        ...
        m_fFader += CORE_GninE_FADER;

        float fFadeTarget = m_bIsIngame ? CORE_GninE_BG_INGAME_ALPHA : CORE_GninE_BG_MAX_ALPHA;

        m_pFiller->SetAlpha(Clamp<float>(0.f, m_fFader, CORE_GninE_BG_MAX_ALPHA));
        m_pFiller2->SetAlpha(Clamp<float>(0.f, m_fFader, CORE_GninE_BG_MAX_ALPHA));
        m_pCanvas->SetAlpha(Clamp<float>(0.f, m_fFader, CORE_GninE_BG_MAX_ALPHA));
        m_pBackground->SetAlpha(Clamp<float>(0.f, m_fFader, CORE_GninE_BG_MAX_ALPHA));
    ...
    else if (m_ucFade == FADE_OUT)
    {
        ...
        m_pFiller->SetAlpha(Clamp(0.f, m_fFader, CORE_GninE_BG_MAX_ALPHA));
        m_pFiller2->SetAlpha(Clamp(0.f, m_fFader, CORE_GninE_BG_MAX_ALPHA));
        m_pCanvas->SetAlpha(Clamp(0.f, m_fFader, CORE_GninE_BG_MAX_ALPHA));
        m_pBackground->SetAlpha(Clamp(0.f, m_fFader, CORE_GninE_BG_MAX_ALPHA));
...
void CMainMenu::SetIsIngame(bool bIsIngame)
{
    ...
    if (m_bIsIngame != bIsIngame)
    {
        ...
        else
        {
            ...
            m_menuAY = fTopItemCentre - fTopItemSize * (CORE_GninE_HOVER_SCALE / CORE_GninE_NORMAL_SCALE) * 0.5f;
            m_menuAY += BODGE_FACTOR_1;
...
sMenuItem* CMainMenu::CreateItem(unsigned char menuType, const char* szFilename, CVector2D vecRelPosition)
{
    ...
    m_menuBY = (iPosY + (iSizeY * CORE_GninE_HOVER_SCALE) / 2) + m_iYOff;
    m_menuBY += BODGE_FACTOR_2;

    // Reduced their size down to unhovered size.
    iSizeX = iSizeX * CORE_GninE_NORMAL_SCALE;
    iSizeY = iSizeY * CORE_GninE_NORMAL_SCALE;
    ...
    pImage->SetAlpha(CORE_GninE_NORMAL_ALPHA);
...
bool CMainMenu::SetItemHoverProgress(sMenuItem* pItem, float fProgress, bool bHovering)
{
    ...
    float fTargetScale = (CORE_GninE_HOVER_SCALE - CORE_GninE_NORMAL_SCALE) * (fProgress) + CORE_GninE_NORMAL_SCALE;

    // Work out our current progress based upon the alpha value now
    pItem->image->SetAlpha((CORE_GninE_HOVER_ALPHA - CORE_GninE_NORMAL_ALPHA) * (fProgress) + CORE_GninE_NORMAL_ALPHA);
...
void CMainMenu::SetNewsHeadline(int iIndex, const SString& strHeadline, const SString& strDate, bool bIsNew)
{
    ...
    if (pItem->GetSize(false).fX < pItem->GetTextExtent())
    {
        ...
        for (char i = 0; i < CORE_GninE_NEWS_ITEMS; i++)
        {
