                                      ...
                                      D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DDevice9** ppReturnedDeviceInterface)
{
    ...
    // Change the window title to GninE: San Andreas
    #ifdef GninE_DEBUG
    SetWindowTextW(hFocusWindow, MbUTF8ToUTF16("GninE: San Andreas [DEBUG]").c_str());
    #else
    SetWindowTextW(hFocusWindow, MbUTF8ToUTF16("GninE: San Andreas").c_str());
    #endif
                                 ...
                                 D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DDevice9** ppReturnedDeviceInterface)
{
    ...
    if (hResult != D3D_OK)
    {
        // Handle fatal error
        SString strMessage;
        strMessage += "There was a problem starting GninE:SA\n\n";
                                  ...
                                  D3DPRESENT_PARAMETERS* pPresentationParameters, IDirect3DDevice9** ppReturnedDeviceInterface)
{
    ...
    if (hResult != D3D_OK)
        WriteDebugEvent(SString("GninE CreateDevice failed: %08x", hResult));
    else
        WriteDebugEvent("GninE CreateDevice succeeded");
