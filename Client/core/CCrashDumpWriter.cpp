...
void CCrashDumpWriter::DumpCoreLog(CExceptionInformation* pExceptionInformation)
{
    ...
    fclose(File::Fopen(CalcGninESAPath("GninE\\core.log.flag"), "w"));

    // Write a log with the generic exception information
    FILE* pFile = File::Fopen(CalcGninESAPath("GninE\\core.log"), "a+");
    if (pFile)
    {
        // Header
        ...
        SString strGninEVersionFull = SString("%s.%s", GninE_DM_BUILDTAG_LONG, *GetApplicationSetting("GninE-version-ext").SplitRight(".", NULL, -2));

        SString strInfo;
        strInfo += SString("Version = %s\n", strGninEVersionFull.c_str());
...
void CCrashDumpWriter::DumpMiniDump(_EXCEPTION_POINTERS* pException, CExceptionInformation* pExceptionInformation)
{
    ...
    if (hDll)
    {
        ...
        {
            // Create the file
            HANDLE hFile = CreateFile(CalcGninESAPath("GninE\\core.dmp"), GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
            if (hFile == INVALID_HANDLE_VALUE)
                AddReportLog(9203, SString("CCrashDumpWriter::DumpMiniDump - Could not create '%s'", *CalcGninESAPath("GninE\\core.dmp")));
            ...
            if (hFile != INVALID_HANDLE_VALUE)
            {
                ...
                CreateDirectory(CalcGninESAPath("GninE\\dumps"), 0);
                CreateDirectory(CalcGninESAPath("GninE\\dumps\\private"), 0);
                ...
                // Get path to GninE dir
                SString strPathCode;
                {
                    std::vector<SString> parts;
                    PathConform(CalcGninESAPath("")).Split(PATH_SEPERATOR, parts);
                    for (uint i = 0; i < parts.size(); i++)
                    {
                        if (parts[i].CompareI("Program Files"))
                            ...
                            strPathCode += "Px";
                        else if (parts[i].CompareI("GninE San Andreas"))
                            strPathCode += "Mt";
                        else if (parts[i].BeginsWithI("GninE San Andreas"))
                            strPathCode += "Mb";
                        else
                ...
                SString strFilename("GninE\\dumps\\private\\client_%s_%s_%08x_%x_%s_%08X_%04X_%03X_%s_%04d%02d%02d_%02d%02d.dmp", strGninEVersionFull.c_str(),
                ...
                // Copy the file
                CopyFile(CalcGninESAPath("GninE\\core.dmp"), strPathFilename, false);
                ...
                logfileContent.LoadFromFile(CalcGninESAPath(PathJoin("GninE", "logs", "logfile.txt")));
                ...
                reportLogContent.LoadFromFile(PathJoin(GetGninEDataPath(), "report.log"));
...
{
    // GninE Error Reporter is now integrated into the launcher
    ...
    SString strGninESAPath = GetGninESABaseDir();
...
#endif
    SString strFile = strGninESAPath + "\\" + GninE_EXE_NAME;
...
void CCrashDumpWriter::GetDxInfo(CBuffer& buffer)
{
    ...
    stream.Write(status.videoMemoryKB.iFreeForGninE);
