...
public:
    ...
    CTimingCheckpoints()
    {
        ...
        m_LogFileName = CalcGninESAPath("timings.log");
    }
    ...
    void BeginTimingCheckpoints()
    {
        ...
        if (bEnabled != m_bEnabled)
        {
            ...
            {
                ClearLog();
                AppendLog(SString("Started timing checkpoints [Ver:%d.%d.%d-%d.%05d] [Date:%s]", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR,
                                  GninESA_VERSION_MAINTENANCE, GninESA_VERSION_TYPE, GninESA_VERSION_BUILD, *GetLocalTimeString(true).SplitLeft(" ")));
            }
            else
