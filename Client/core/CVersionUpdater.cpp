...
void CVersionUpdater::GetBrowseVersionMaps(std::map<SString, int>& outBlockedVersionMap, std::map<SString, int>& outAllowedVersionMap)
{
    ...
    //
    {
        ...
        for (uint i = 0; i < dataInfoSet.size(); i++)
        {
            ...
            if (pstrVersion && *pstrVersion == GninE_DM_ASE_VERSION)
            {
...
{
    outSaveLocationList.push_back(PathJoin(GetGninEDataPath(), "upcache", strFilename));
    outSaveLocationList.push_back(PathJoin(GetGninETempPath(), "upcache", strFilename));
    outSaveLocationList.push_back(GetGninETempPath() + strFilename);
...
void CVersionUpdater::_CheckSidegradeRequirements()
{
    ...
    if (bVersionMatch && bLaunchPathValid)
    {
        ...
        SString         strCoreDll = PathJoin(ExtractPath(m_strSidegradePath), "GninE", "core.dll");
...
{
    SString strURL("GninEsa://%s:%s@%s", *m_strSidegradeName, *m_strSidegradePassword, *m_strSidegradeHost);
...
{
    GetQuestionBox().Reset();
    GetQuestionBox().SetTitle(SString(_("GninE:SA %s required"), *m_strSidegradeVersion));
    GetQuestionBox().SetMessage(SString(_("An updated version of GninE:SA %s is required to join the selected server.\n\n"
                                          "Do you want to download and install GninE:SA %s ?"),
...
void CVersionUpdater::_DialogSidegradeLaunchQuestion()
{
    GetQuestionBox().Reset();
    GetQuestionBox().SetTitle(SString(_("GninE:SA %s required"), *m_strSidegradeVersion));
    GetQuestionBox().SetMessage(SString(_("Do you want to launch GninE:SA %s and connect to this server ?"), *m_strSidegradeVersion));
    GetQuestionBox().SetButton(0, _("No"));
    GetQuestionBox().SetButton(1, _("Yes"));
    GetQuestionBox().Show();
    _PollQuestionNoYes();
...
void CVersionUpdater::_DialogServerSaysUpdateQuestion()
{
    ...
    GetQuestionBox().SetMessage(_("To join this server, you must update GninE.\n\n Do you want to update now ?"));
    GetQuestionBox().SetButton(0, _("No"));
    GetQuestionBox().SetButton(1, _("Yes"));
    GetQuestionBox().Show();
    _PollQuestionNoYes();
...
void CVersionUpdater::_DialogDataFilesQuestion()
{
    ...
    GetQuestionBox().SetMessage(_("Some GninE:SA data files are missing.\n\n\nPlease reinstall GninE:SA"));
    GetQuestionBox().SetButton(0, _("Ok"));
    GetQuestionBox().Show();
    _PollAnyButton();
}
...
static bool GetRevisionFromFileName(std::string_view fileName, std::uint32_t& revision)
{
    ...
    // Example:                          GninEsa-1.5.9-rc-21519-0-000-files-all-cksummed.rar
...
void CVersionUpdater::_ProcessPatchFileQuery()
{
    ...
    if (!m_JobInfo.strStatus.empty() && m_JobInfo.strStatus != "noupdate" && !_strnicmp("GninEsa-", m_JobInfo.strFilename.c_str(), 6))
    {
        uint32_t revision{};

        if (GetRevisionFromFileName(m_JobInfo.strFilename, revision) && revision < GninESA_VERSION_BUILD)
        {
            ...
            SString playerVersion("%d.%d.%d-%d.%05d.%d.%03d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR, GninESA_VERSION_MAINTENANCE, GninESA_VERSION_TYPE,
                                  GninESA_VERSION_BUILD, netRev, netRel);
    ...
    if (m_JobInfo.slim.iFilesize > 0)
    {
        ...
        for (uint i = 0; i < m_JobInfo.slim.dependencyInfoMap.size(); i++)
        {
            ...
            if (dependencyItem.strName == "dependency")
            {
                ...
                {
                    SString strPathFilename = CalcGninESAPath(strFilename);
...
int CVersionUpdater::DoSendDownloadRequestToNextServer()
{
    ...
    SString        strPlayerVersion("%d.%d.%d-%d.%05d.%d.%03d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR, GninESA_VERSION_MAINTENANCE, GninESA_VERSION_TYPE,
                             GninESA_VERSION_BUILD, usNetRev, usNetRel);
