...
void CRenderItemManager::OnDeviceCreate(IDirect3DDevice9* pDevice, float fViewportSizeX, float fViewportSizeY)
{
    ...
    m_iVideoCardMemoryKBForGninETotal = (m_iVideoCardMemoryKBTotal - (64 * 1024)) * 4 / 5;
    m_iVideoCardMemoryKBForGninETotal = std::max(0, m_iVideoCardMemoryKBForGninETotal);
                                                          ...
                                                          bool bForce)
{
    ...
    // Include in memory stats only if render target is not for GninE internal use
...
bool CRenderItemManager::SaveDefaultRenderTarget()
{
    ...
    CGraphics::GetSingleton().MaybeEnteringGninERenderZone();
    return true;
}
...
bool CRenderItemManager::RestoreDefaultRenderTarget()
{
    ...
    if (m_pDefaultD3DRenderTarget)
    {
        ...
        // Do this in case dxSetRenderTarget is being called from some unexpected place
        CGraphics::GetSingleton().MaybeLeavingGninERenderZone();
    }
...
void CRenderItemManager::UpdateMemoryUsage()
{
    ...
    m_iMemoryKBFreeForGninE = m_iVideoCardMemoryKBForGninETotal;
    m_iMemoryKBFreeForGninE -= m_iFontMemoryKBUsed / 2;
    m_iMemoryKBFreeForGninE -= m_iTextureMemoryKBUsed / 4;
    m_iMemoryKBFreeForGninE -= m_iRenderTargetMemoryKBUsed;
    m_iMemoryKBFreeForGninE = std::max(0, m_iMemoryKBFreeForGninE);
}
...
void CRenderItemManager::GetDxStatus(SDxStatus& outStatus)
{
    ...
    // Memory usage
    outStatus.videoMemoryKB.iFreeForGninE = m_iMemoryKBFreeForGninE;
    ...
    if (m_TestMode == DX_TEST_MODE_NO_MEM)
        outStatus.videoMemoryKB.iFreeForGninE = 0;

    if (m_TestMode == DX_TEST_MODE_LOW_MEM)
        outStatus.videoMemoryKB.iFreeForGninE = 1;
