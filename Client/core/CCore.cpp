...
using SharedUtil::CalcGninESAPath;
using namespace std;
...
{
    m_strModInstallRoot = CalcGninESAPath(PathJoin("mods", szModName));
...
bool CCore::CheckDiskSpace(uint uiResourcesPathMinMB, uint uiDataPathMinMB)
{
    ...
    {
        SString strMessage(_("GninE:SA cannot continue because drive %s does not have enough space."), *strDriveWithNoSpace);
...
void CCore::ApplyHooks()
{
    ...
    // m_pFileSystemHook->RedirectFile ( "main.scm", "../../GninE/gtafiles/main.scm" );
...
void LoadModule(CModuleLoader& m_Loader, const SString& strName, const SString& strModuleName)
{
    ...
    if (CalcGninESAPath("GninE").CompareI(strDllDirectory) == false)
    {
        AddReportLog(3119, SString("DllDirectory wrong:  DllDirectory:'%s'  Path:'%s'", *strDllDirectory, *CalcGninESAPath("GninE")));
        SetDllDirectory(CalcGninESAPath("GninE"));
    }
...
#endif
    m_Loader.LoadModule(CalcGninESAPath(PathJoin("GninE", strModuleFileName)));
...
void CCore::CreateNetwork()
{
    ...
    if (!pfnCheckCompatibility || !pfnCheckCompatibility(GninE_DM_CLIENT_NET_MODULE_VERSION, NULL))
    {
        // net.dll doesn't like our version number
        ulong ulNetModuleVersion = 0;
        pfnCheckCompatibility(1, &ulNetModuleVersion);
        SString strMessage("Network module not compatible! (Expected 0x%x, got 0x%x)", GninE_DM_CLIENT_NET_MODULE_VERSION, ulNetModuleVersion);
    ...
    // Set GninE version for report log here
    SetApplicationSetting("GninE-version-ext", SString("%d.%d.%d-%d.%05d.%d.%03d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR, GninESA_VERSION_MAINTENANCE,
                                                     GninESA_VERSION_TYPE, GninESA_VERSION_BUILD, m_pNet->GetNetRev(), m_pNet->GetNetRel()));
    char szSerial[64];
...
{
    if (!m_pXML)
        m_pXML = CreateModule<CXML>(m_XMLModule, "XML", "xmll", "InitXMLInterface", *CalcGninESAPath("GninE"));

    if (!m_pConfigFile)
    {
        // Load config XML file
        m_pConfigFile = m_pXML->CreateXML(CalcGninESAPath(GninE_CONFIG_PATH));
        if (!m_pConfigFile)
        {
            assert(false);
            return;
    ...
    if (m_pGame->GetSystemState() == 7)            // GS_FRONTEND
    {
        ...
        if (WaitForMenu >= 250)
        {
            if (m_bFirstFrame)
            {
                ...
                // Parse the command line
                // Does it begin with GninEsa://?
                if (m_szCommandLineArgs && strnicmp(m_szCommandLineArgs, "GninEsa://", 8) == 0)
                {
...
void CCore::HandleCrashDumpEncryption()
{
    ...
    SString   strDumpDirPath = CalcGninESAPath("GninE\\dumps");
    ...
    {
        SString strCoreLogPathFilename = CalcGninESAPath("GninE\\core.log");
