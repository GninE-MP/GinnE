...
#define CORE_GninE_FILLER "cgui\\images\\GninE_filler.png"
...
void CSettings::CreateGUI()
{
    ...
    else
    {
        ...
        pFiller->LoadFromFile(CORE_GninE_FILLER);
     ...
     *  Audio tab
     **/
    fIndentX = pManager->CGUI_GetMaxTextExtent("default-normal", _("Master volume:"), _("Radio volume:"), _("SFX volume:"), _("GninE volume:"),
    ...
    vecTemp.fX = 11;
    m_pLabelGninEVolume = reinterpret_cast<CGUILabel*>(pManager->CreateLabel(pTabAudio, _("GninE volume:")));
    m_pLabelGninEVolume->SetPosition(CVector2D(vecTemp.fX, vecTemp.fY + 29.0f));
    m_pLabelGninEVolume->GetPosition(vecTemp, false);
    m_pLabelGninEVolume->AutoSize();

    m_pAudioGninEVolume = reinterpret_cast<CGUIScrollBar*>(pManager->CreateScrollBar(true, pTabAudio));
    m_pAudioGninEVolume->SetPosition(CVector2D(vecTemp.fX + fIndentX + 5.0f, vecTemp.fY));
    m_pAudioGninEVolume->GetPosition(vecTemp, false);
    m_pAudioGninEVolume->SetSize(CVector2D(160.0f, 20.0f));
    m_pAudioGninEVolume->GetSize(vecSize, false);
    m_pAudioGninEVolume->SetProperty("StepSize", "0.01");

    m_pLabelGninEVolumeValue = reinterpret_cast<CGUILabel*>(pManager->CreateLabel(pTabAudio, "0%"));
    m_pLabelGninEVolumeValue->SetPosition(CVector2D(vecTemp.fX + vecSize.fX + 5.0f, vecTemp.fY));
    m_pLabelGninEVolumeValue->GetPosition(vecTemp, false);
    m_pLabelGninEVolumeValue->AutoSize("100%");
    m_pLabelGninEVolumeValue->GetSize(vecSize, false);
    ...
    m_pCheckBoxMuteGninE = reinterpret_cast<CGUICheckBox*>(pManager->CreateCheckBox(pTabAudio, _("Mute GninE sounds when minimized"), true));
    m_pCheckBoxMuteGninE->SetPosition(CVector2D(vecTemp.fX, vecTemp.fY + 20.0f));
    m_pCheckBoxMuteGninE->GetPosition(vecTemp, false);
    m_pCheckBoxMuteGninE->AutoSize(NULL, 20.0f);
    ...
    {
#ifndef GninE_DEBUG            // Don't hide when debugging
    ...
    m_pAudioGninEVolume->SetOnScrollHandler(GUI_CALLBACK(&CSettings::OnGninEVolumeChanged, this));
    ...
    m_pCheckBoxMuteGninE->SetClickHandler(GUI_CALLBACK(&CSettings::OnGninEMuteMinimizedChanged, this));
...
{
    SString strMessage = _("Some settings will be changed when you next start GninE");
...
{
    float fMasterVolume = 0, fGninEVolume = 0, fVoiceVolume = 0;

    CVARS_GET("mastervolume", fMasterVolume);
    CVARS_GET("GninEvolume", fGninEVolume);
    ...
    m_pAudioGninEVolume->SetScrollPosition(fGninEVolume);
    ...
    CVARS_GET("mute_GninE_when_minimized", m_bMuteGninE);
    ...
    m_pCheckBoxMuteGninE->SetSelected(m_bMuteGninE);
    ...
    m_pCheckBoxMuteGninE->SetEnabled(!m_bMuteMaster);
...
bool CSettings::OnAudioDefaultClick(CGUIElement* pElement)
{
    ...
    CVARS_SET("mute_GninE_when_minimized", false);
...
bool CSettings::OnCancelButtonClick(CGUIElement* pElement)
{
    ...
    CVARS_SET("GninEvolume", m_fOldGninEVolume);
    ...
    CVARS_SET("mute_GninE_when_minimized", m_bOldMuteGninE);
    ...
    m_pCheckBoxMuteGninE->SetSelected(m_bOldMuteGninE);
    ...
    m_pCheckBoxMuteGninE->SetEnabled(!m_bOldMuteMaster);
...
void CSettings::LoadData()
{
    ...
    CVARS_GET("mute_GninE_when_minimized", m_bOldMuteGninE);
    ...
    CVARS_GET("GninEvolume", m_fOldGninEVolume);
    ...
    m_fOldGninEVolume = max(0.0f, min(1.0f, m_fOldGninEVolume));
    ...
    m_pAudioGninEVolume->SetScrollPosition(m_fOldGninEVolume);
...
void CSettings::SaveData()
{
    ...
    g_pCore->GetModManager()->TriggerCommand(GninEsa::CMD_ALWAYS_SHOW_TRANSFERBOX, alwaysShowTransferBox);
...
void CSettings::CreateChatColorTab(eChatColorType eType, const char* szName, CGUITabPanel* pParent)
{
    ...
    //
    {
        ...
        if (pPreviewImage->LoadFromFile(CalcGninESAPath("GninE\\cgui\\images\\radarset\\01.png")))
        {
...
{
    CXMLFile* pPresetsFile = CCore::GetSingleton().GetXML()->CreateXML(CalcGninESAPath(CHAT_PRESETS_PATH));
...
{
    std::vector<SString> directories = FindFiles(CalcGninESAPath(SKINS_PATH), false, true);
...
bool CSettings::OnGninEVolumeChanged(CGUIElement* pElement)
{
    int iVolume = m_pAudioGninEVolume->GetScrollPosition() * 100.0f;
    m_pLabelGninEVolumeValue->SetText(SString("%i%%", iVolume).c_str());

    CVARS_SET("GninEvolume", m_pAudioGninEVolume->GetScrollPosition());
...
bool CSettings::OnMasterMuteMinimizedChanged(CGUIElement* pElement)
{
    ...
    m_pCheckBoxMuteGninE->SetEnabled(!bSelected);
...
bool CSettings::OnGninEMuteMinimizedChanged(CGUIElement* pElement)
{
    CVARS_SET("mute_GninE_when_minimized", m_pCheckBoxMuteGninE->GetSelected());
    return true;
}
    ...
    if (m_pCheckBoxCustomizedSAFiles->GetSelected())
    {
        ...
        strMessage +=
            _("Some files in your GTA:SA data directory are customized."
              "\nGninE will only use these modified files if this check box is ticked."
    ...
    if (m_pCheckBoxDPIAware->GetSelected() && !shownWarning)
    {
        ...
        SStringX strMessage(
            _("Enabling DPI awareness is an experimental feature and\n"
              "we only recommend it when you play GninE:SA on a scaled monitor.\n"
...
bool CSettings::OnShowAdvancedSettingDescription(CGUIElement* pElement)
{
    ...
    if (pLabel && pLabel == m_pPriorityLabel || pComboBox && pComboBox == m_pPriorityCombo)
        ...
        strText = std::string(_("Packet tag:")) + " " + std::string(_("Tag network packets to help ISPs identify GninE traffic."));
        ...
        strText = std::string(_("16-bit color:")) + " " + std::string(_("Enable 16 bit color modes - Requires GninE restart"));
