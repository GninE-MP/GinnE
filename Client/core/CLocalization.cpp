...
#define GninE_LOCALE_TEXTDOMAIN       "client"
...
CLocalization::CLocalization(const SString& strLocale, const SString& strLocalePath)
{
    ...
    m_DictManager.add_directory(strLocalePath.empty() ? CalcGninESAPath(GninE_LOCALE_DIR) : strLocalePath);
...
CLanguage* CLocalization::GetLanguage(SString strLocale)
{
    ...
    if (!pLanguage)
    {
        Language Lang = Language::from_name(strLocale);
        Lang = Lang ? Lang : Language::from_name("en_US");
        pLanguage = new CLanguage(m_DictManager.get_dictionary(Lang, GninE_LOCALE_TEXTDOMAIN), Lang.str(), Lang.get_name());
...
std::vector<SString> CLocalization::GetAvailableLocales()
{
    ...
    for (const auto& language : m_DictManager.get_languages(GninE_LOCALE_TEXTDOMAIN))
        // To avoid duplicates
...
// Global interface
//
GninEEXPORT CLocalizationInterface* __cdecl L10n_CreateLocalization(SString strLocale)
{
