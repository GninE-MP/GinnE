...
// Update master info
#define UPDATER_MASTER_URL1 "http://updatesa.GninEsa.com/sa/master/?v=%VERSION%&id=%ID%"
#define UPDATER_MASTER_URL2 "http://updatesa.gnine.com/sa/master/?v=%VERSION%&id=%ID%"
...
<masterroot>
    <master>
        ...
        <serverlist>
            <server>http://updatesa.GninEsa.com/sa/master/?v=%VERSION%&amp;id=%ID%</server>
            <server>http://updatesa.gnine.com/sa/master/?v=%VERSION%&amp;id=%ID%</server>
        </serverlist>
        <interval>7d</interval>
    </master>
    <version>
        <serverlist>
            <server priority="3">http://updatesa.GninEsa.com/sa/version/?v=%VERSION%&amp;id=%ID%&amp;ty=%TYPE%&amp;da=%DATA%&amp;be=%BETA%&amp;re=%REFER%</server>
            <server
priority="3">http://updatesa.gnine.com/sa/version/?v=%VERSION%&amp;id=%ID%&amp;ty=%TYPE%&amp;da=%DATA%&amp;be=%BETA%&amp;re=%REFER%</server>
        </serverlist>
        <interval>12h</interval>
    </version>
    <report>
        <serverlist>
            <server>http://updatesa.GninEsa.com/sa/report/?v=%VERSION%&amp;id=%ID%</server>
            <server>http://updatesa.gnine.com/sa/report/?v=%VERSION%&amp;id=%ID%</server>
        </serverlist>
        <interval>12h</interval>
    </report>
    <crashdump>
        <serverlist>
            <server priority="3">http://updatesa.GninEsa.com/sa/crashdump/?v=%VERSION%&amp;id=%ID%&amp;file=%FILE%</server>
            <server priority="4">http://updatesa.gnine.com/sa/crashdump/?v=%VERSION%&amp;id=%ID%&amp;file=%FILE%</server>
        </serverlist>
        <interval>12h</interval>
    ...
    <gtadatafiles>
        <serverlist>
            <server priority="4">http://updatesa.GninEsa.com/sa/gtadatafiles/?v=%VERSION%&amp;id=%ID%</server>
            <server priority="3">http://updatesa.gnine.com/sa/gtadatafiles/?v=%VERSION%&amp;id=%ID%</server>
        </serverlist>
    </gtadatafiles>
    <trouble>
        <serverlist>
            <server>http://updatesa.GninEsa.com/sa/trouble/?v=%VERSION%&amp;id=%ID%&amp;tr=%TROUBLE%</server>
        </serverlist>
    </trouble>
</masterroot>
...
<versioncheckroot>
    ...
    <dialog>
        ...
        <msg>There is an update for the version of GninE you are using.
...
DO YOU WANT TO DOWNLOAD AND INSTALL THE LATEST VERSION ?
    ...
    <file>
        <name>GninEsa-1.0.4-rc-2021-6-files-net.exe</name>
        <size>213857</size>
        <md5>1585F5C5FDF77AD3565A75608E0263DD</md5>
    </file>
    <serverlist>
        <server priority="3">http://updatesa1.GninEsa.com/mirror/GninEsa-1.0.4-rc-2021-6-files-net.exe</server>
        <server priority="3">http://updatesa2.GninEsa.com/mirror/GninEsa-1.0.4-rc-2021-6-files-net.exe</server>
        <server priority="2">http://updatesa3.GninEsa.com/mirror/GninEsa-1.0.4-rc-2021-6-files-net.exe</server>
    </serverlist>
    <reportsettings>
...
namespace
{
    ...
    public:
        ...
        CXMLNode* SetFromData(char* data, uint uiSize)
        {
            ...
            // Try to save
            m_strTempFileName = MakeUniquePath(PathJoin(GetGninEDataPath(), "temp", "buffer.xml"));
    ...
    public:
        ...
        {
            if (strIdFilter == "-all" && FileExists(CalcGninESAPath("_keep_report_")))
                return;
            if (FileExists(CalcGninESAPath("_keep_report_all_")))
                return;
