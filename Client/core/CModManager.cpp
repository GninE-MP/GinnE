...
CModManager::CModManager()
{
    ...
    // Load the modlist from the folders in "GninE/mods"
    InitializeModList(CalcGninESAPath("mods\\"));
}
...
void CModManager::InitializeModList(const char* szModFolderPath)
{
    ...
    // Set the working directory to the GninE folder
    CFilePathTranslator filePathTranslator;
    filePathTranslator.SetCurrentWorkingDirectory("GninE");
...
void CModManager::VerifyAndAddEntry(const char* szModFolderPath, const char* szName)
{
    ...
    {
        // Put together a modpath string and a GninE-relative path to Client(_d).dll
