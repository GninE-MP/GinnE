...
int _declspec(dllexport) InitCEF()
{
    ...
    // Extract GninE path and set DLL directory (absolute path is required here)
    size_t       pos = currentFileName.find_last_of(L'\\');
    std::wstring GninEPath = currentFileName.substr(0, pos - 3);            // Strip "CEF"
    SetDllDirectory(GninEPath.c_str());
