...
const SDialogItemInfo g_CrashedDialogItems[] = {
    {0, 0, _td("GninE: San Andreas has encountered a problem")},
    {IDC_CRASH_HEAD, 0, _td("Crash information")},
    {IDC_SEND_DUMP_CHECK, 0, _td("Tick the check box to send this crash info to GninE devs using the 'internet'")},
    {IDC_SEND_DESC_STATIC, 0, _td("Doing so will increase the chance of this crash being fixed.")},
    {IDC_RESTART_QUESTION_STATIC, 1, _td("Do you want to restart GninE: San Andreas ?")},
...
const SDialogItemInfo g_GraphicsDllDialogItems[] = {
    {0, 0, _td("GninE: San Andreas - Warning")},
    {IDC_D3DDLL_TEXT1, 0, _td("Your Grand Theft Auto: San Andreas install directory contains these files:")},
    {IDC_D3DDLL_TEXT2, 0,
     _td("These files are not required and may interfere with the graphical features in this version of GninE:SA.\n\n"
...
const SDialogItemInfo g_NoAvDialogItems[] = {
    {0, 0, _td("GninE: San Andreas")},
    {IDC_NOAV_TEXT1, 0, _td("Warning: Could not detect anti-virus product")},
    {IDC_NOAV_TEXT2, 0,
     _td("GninE could not detect an anti-virus on your PC.\n\n"
         "Viruses interfere with GninE and degrade your gameplay experience.\n\n"
...
void ShowSplash(HINSTANCE hInstance)
{
    ...
    {
        WindowScope window(CreateWindowEx(WS_EX_LAYERED, splashWindowClass.lpszClassName, "GninE Launcher", WS_POPUP | WS_VISIBLE, 0, 0, 0, 0, NULL,
                                          NULL, hInstance, NULL));
...
void ShowOptimusDialog(HINSTANCE hInstance)
{
    ...
    if (!GetInstallManager()->UpdateOptimusSymbolExport())
    {
        ...
        ShellExecuteNonBlocking("open", PathJoin(GetGninESAPath(), GninE_EXE_NAME));
...
void TestDialogs()
{
    ...
    ShowProgressDialog( g_hInstance, "GninE: San Andreas" );
    for ( uint i = 0 ; i < 100 ; i++ )
    {
