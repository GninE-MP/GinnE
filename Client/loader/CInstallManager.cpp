...
namespace
{
    ...
    {
        UpdateGninEVersionApplicationSetting();
...
//////////////////////////////////////////////////////////
//
// CInstallManager::SetGninESAPathSource
//
// Figure out and set the path that GetGninESAPath() should use
//
//////////////////////////////////////////////////////////
void CInstallManager::SetGninESAPathSource(const SString& strCommandLineIn)
{
    ...
    if (m_pSequencer->GetVariable(INSTALL_LOCATION) == "far")
        ::SetGninESAPathSource(true);
    else
        ::SetGninESAPathSource(false);
}
...
SString CInstallManager::Continue()
{
    ...
    AddReportLog(1041, SString("* Launch * pid:%d '%s' GninESAPath set from %s '%s'", dwProcessId, GetLaunchPathFilename().c_str(), GotPathFrom.c_str(),
                               GetGninESAPath().c_str()));
...
SString CInstallManager::GetLauncherPathFilename()
{
    ...
    SString strResult = PathJoin(strLocation == "far" ? GetSystemCurrentDirectory() : GetGninESAPath(), GninE_EXE_NAME);
...
SString CInstallManager::_ChangeToAdmin()
{
    ...
    {
        MessageBoxUTF8(
            NULL, SString(_("GninE:SA needs Administrator access for the following task:\n\n  '%s'\n\nPlease confirm in the next window."), *m_strAdminReason),
            "GninE: San Andreas", MB_OK | MB_ICONINFORMATION | MB_TOPMOST);
        ...
        CreateSingleInstanceMutex();
        ClearIsBlockingUserProcess();
        MessageBoxUTF8(NULL, SString(_("GninE:SA could not complete the following task:\n\n  '%s'\n"), *m_strAdminReason),
                       "GninE: San Andreas" + _E("CL01"), MB_OK | MB_ICONWARNING | MB_TOPMOST);
    }
    return "fail";
}
...
{
    // If a new "GninE.exe" exists, let that complete the install
    if (m_pSequencer->GetVariable(INSTALL_LOCATION) == "far")
    {
        ReleaseSingleInstanceMutex();
...
SString CInstallManager::_InstallFiles()
{
    ...
    if (!InstallFiles(m_pSequencer->GetVariable(HIDE_PROGRESS) != "yes"))
    {
        ...
        m_strAdminReason = _("Install updated GninE:SA files");
        return "fail";
    }
    else
    {
...
{
    MessageBoxUTF8(0, SString(_("GninE has not been installed properly, please reinstall. %s"), *strExtraInfo), _("Error") + _E("CL03"),
...
SString CInstallManager::_PrepareLaunchLocation()
{
    ...
    const fs::path GninEDir = GetGninERootDirectory() / "GninE";
    ...
    // Copy GninE dependencies to our launch directory.
    for (const char* fileName : {LOADER_PROXY_DLL_NAME})
    {
        const fs::path sourcePath = GninEDir / fileName;
        const fs::path targetPath = launchDir / fileName;
        ...
        if (std::error_code ec; !SafeCopyFile(sourcePath, targetPath, ec))
        {
            ...
            if (isAdmin)
            {
                ...
                {
                    SString strMessage(_("GninE:SA cannot launch because copying a file failed:"));
                    strMessage += "\n\n" + targetPath.u8string();
                    BrowseToSolution("copy-files", ASK_GO_ONLINE, strMessage);
                }
                else
                {
                    SString strMessage(_("GninE:SA cannot launch because an GninE:SA file is incorrect or missing:"));
                    strMessage += "\n\n" + sourcePath.u8string();
                    BrowseToSolution("GninE-datafiles-missing", ASK_GO_ONLINE, strMessage);
                }
            ...
            {
                m_strAdminReason = _("Copy GninE:SA files");
                return "fail";
            }
        }
    }
...
SString CInstallManager::_ProcessGtaPatchCheck()
{
    ...
    {
        SString strMessage(_("GninE:SA cannot launch because a GTA:SA file is incorrect or missing:"));
...
SString CInstallManager::_ProcessGtaVersionCheck()
{
    ...
    if (!patcher.Load(ec))
    {
        ...
        if (!gtasa.GenerateFile("data", gtaExePath, ec))
        {
            ...
            if (isAdmin)
            {
                SString strMessage(_("GninE:SA cannot launch because the GTA:SA executable is incorrect or missing:"));
                strMessage += "\n\n" + gtaExePath.u8string();
                strMessage +=
                    "\n\n" +
                    _("Please check your anti-virus for a false-positive detection, try to add an exception for the GTA:SA executable and restart GninE:SA.");
        ...
        if (!patcher.Load(ec))
        {
            ...
            if (isAdmin)
            {
                SString strMessage(_("GninE:SA cannot launch because the GTA:SA executable is not loadable:"));
                strMessage += "\n\n" + gtaExePath.u8string();
                BrowseToSolution(SString("gengta_error&code=%d", ec.value()), ASK_GO_ONLINE, strMessage);
                return "quit";
            }
    ...
    if (!patcher.ApplyPatches(ec))
    {
        ...
        if (isAdmin)
        {
            SString strMessage(_("GninE:SA cannot launch because patching GTA:SA has failed:"));
            strMessage += "\n\n" + gtaExePath.u8string();
...
SString CInstallManager::_ProcessLayoutChecks()
{
    ...
    for (uint i = 0; i < 2; i++)
    {
        SString strGninEDataPath = i ? GetGninEDataPathCommon() : GetGninEDataPath();

        if (!DirectoryExists(strGninEDataPath))
            ShowLayoutError("[Data directory not present]");            // Can't find directory

        SString strTestFilePath = PathJoin(strGninEDataPath, "testdir", "testfile.txt");
    ...
    {
        SString strTestFilePath = CalcGninESAPath(PathJoin("GninE", "writetest.txt"));
    ...
    // If news/temp/upcache folder doesn't exist in new, but does in old place, move it
    {
        ...
        {
            SString strSrc = PathJoin(GetSystemLocalAppDataPath(), "GninE San Andreas " + GetMajorVersionString(), folders[i]);
            SString strDest = PathJoin(GetGninEDataPath(), folders[i]);
    ...
    {
        SString strOldDir = CalcGninESAPath("GninE");
        SString strNewConfigDir = CalcGninESAPath(PathJoin("GninE", "config"));
        SString strNewLogDir = CalcGninESAPath(PathJoin("GninE", "logs"));
    ...
    // Remove superfluous files from GninE subdirectory.
    {
        const SString GninEDir = CalcGninESAPath("GninE");

        for (const char* fileName : {"winmm.dll", "vea.dll", "vog.dll", "vvo.dll", "vvof.dll"})
        {
            SString filePath = PathJoin(GninEDir, fileName);
    ...
    {
        SString strMessage(_("GninE:SA cannot continue because drive %s does not have enough space."), *strDriveWithNoSpace.Left(1));
...
SString CInstallManager::_ProcessLangFileChecks()
{
    ...
    if (strSrcFile.empty())
    {
        ...
        strMessage += "\n\n";
        strMessage += _("If GninE fails to load, please re-install GTA:SA");
        MessageBoxUTF8(NULL, strMessage, "GninE: San Andreas", MB_OK | MB_ICONWARNING | MB_TOPMOST);
        return "ok";
    }
...
SString CInstallManager::_ProcessAppCompatChecks()
{
    ...
    WString strGninEExePathFilename = FromUTF8(GetLaunchPathFilename());
    ...
    struct
    {
        ...
        uint    uiFlags;
                 ...
                 {strGninEExePathFilename, HKEY_CURRENT_USER, 0},
                 {strGninEExePathFilename, HKEY_LOCAL_MACHINE, uiHKLMFlags}};
...
SString CInstallManager::_InstallNewsItems()
{
    ...
    for (uint i = 0; i < keyList.size(); i++)
    {
        ...
        SString strTargetDir = PathJoin(GetGninEDataPath(), "news", strDate);
        MkDir(strTargetDir);
