 ...
 *  PURPOSE:     Handles the installation of updates for GninE
 *
...
static char        ARCHIVE_PASSWORD[] = "GninE";
...
static bool TerminateFileLockingProcesses(const SString& absolutePath, const SString& displayName)
{
    ...
    while (true)
    {
        ...
        int decision = MessageBoxUTF8(
            ...
            "GninE: San Andreas", MB_CANCELTRYCONTINUE | MB_ICONQUESTION | MB_TOPMOST);
...
static SString CreateWritableDirectory(const SString& templateRoot)
{
    ...
    std::vector<SString> candidates{
        ...
        PathJoin(GetGninETempPath(), "upcache", fileName),
    };
    ...
    if (disasterCounter > 0)
    {
        MessageBoxUTF8(nullptr,
                       SString(_("Your installation may be corrupt now.\n\n%zu out of %zu files could not be restored from the backup.\n\nYou should "
                                 "reinstall GninE from www.gnine.com\nor try running the update with administrator rights."),
                               disasterCounter, files.size()),
                       "GninE: San Andreas", MB_OK | MB_ICONERROR | MB_TOPMOST);
...
/**
 * @brief Installs files from the current temporary directory to the GninE installation directory.
...
static int RunInstall()
{
    ...
    const SString targetRoot = PathConform(GetGninESAPath());

    if (!DirectoryExists(targetRoot))
    {
        AddReportLog(5055, SString("RunInstall: GninE directory does not exist: '%s'", targetRoot.c_str()));
        return 1;
    }
...
bool InstallFiles(bool showProgressWindow)
{
    if (showProgressWindow)
        StartPseudoProgress(g_hInstance, "GninE: San Andreas", _("Installing update..."));
    ...
    if (GetOnRestartCommand(strOperation, strFile, strParameters, strDirectory, strShowCmd))
    {
        if (strOperation == "files" || strOperation == "silent")
        {
            ...
            if (!strParameters.Contains("hideprogress"))
                StartPseudoProgress(g_hInstance, "GninE: San Andreas", _("Extracting files..."));
            ...
            if (FileExists(GninE_EXE_NAME_RELEASE))
