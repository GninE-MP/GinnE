...
static SString g_strGninESAPath;
...
std::vector<DWORD> GetOtherGninEProcessList()
{
    ...
    {
        SString strPathFilename = GetProcessPathFilename(processId);
        if (strPathFilename.EndsWith(GninE_EXE_NAME))
            ListAddUnique(result, processId);
    }

    if (DWORD processId = FindProcessId(GninE_EXE_NAME))
        ListAddUnique(result, processId);
...
//
///////////////////////////////////////////////////////////////////////////
bool IsOtherGninERunning()
{
    return !GetOtherGninEProcessList().empty();
}
...
//
///////////////////////////////////////////////////////////////////////////
void TerminateOtherGninEIfRunning()
{
    std::vector<DWORD> processIdList = GetOtherGninEProcessList();

    if (processIdList.size())
    {
        // Try to stop all other GninE process id's
        for (uint i = 0; i < 3 && processIdList.size(); i++)
        {
            for (auto processId : processIdList)
            {
            ...
            Sleep(1000);
            processIdList = GetOtherGninEProcessList();
        }
    }
}
...
auto GetGninERootDirectory() -> std::filesystem::path
{
    static const auto directory = fs::path{FromUTF8(GetGninESAPath())};
    return directory;
}
...
{
    static const auto directory = fs::path{FromUTF8(GetGninEDataPath())} / "GTA San Andreas";
    return directory;
}
...
void SetGninESAPathSource(bool bReadFromRegistry)
{
    if (bReadFromRegistry)
    {
        g_strGninESAPath = GetGninESABaseDir();
    }
    else
    {
        ...
        SetRegistryValue("", "Last Run Path Version", GninE_DM_ASE_VERSION);
        ...
        g_strGninESAPath = strLaunchPath;
    }
}

SString GetGninESAPath()
{
    if (g_strGninESAPath == "")
        SetGninESAPathSource(false);
    return g_strGninESAPath;
}
...
ePathResult GetGamePath(SString& strOutResult, bool bFindIfMissing)
{
    ...
    // Try HKLM "SOFTWARE\\GninE: San Andreas All\\Common\\"
    ...
    for (uint i = 0; i < pathList.size(); i++)
    {
        ...
        if (FileExists(PathJoin(pathList[i], GninE_GTA_KNOWN_FILE_NAME)))
        {
    ...
    // Check browse result
    if (!FileExists(PathJoin(strOutResult, GninE_GTA_KNOWN_FILE_NAME)))
    {
...
void RelaunchAsAdmin(const SString& strCmdLine, const SString& strReason)
{
    ...
    MessageBoxUTF8(NULL, SString(_("GninE:SA needs Administrator access for the following task:\n\n  '%s'\n\nPlease confirm in the next window."), *strReason),
                   "GninE: San Andreas", MB_OK | MB_ICONINFORMATION | MB_TOPMOST);
    ReleaseSingleInstanceMutex();
    ShellExecuteNonBlocking("runas", PathJoin(GetGninESAPath(), GninE_EXE_NAME), strCmdLine);
}
...
HMODULE GetLibraryHandle(const SString& strFilename, DWORD* pdwOutLastError)
{
    if (!hLibraryModule)
    {
        ...
        SString strLibPath = PathJoin(GetLaunchPath(), "GninE");
...
//
// Make sure "GninE-version-ext" is correct. eg "1.0.4-9.01234.2.000"
//
/////////////////////////////////////////////////////////////////////
void UpdateGninEVersionApplicationSetting(bool bQuiet)
{
#ifdef GninE_DEBUG
    ...
    std::vector<SString> parts;
    GetApplicationSetting("GninE-version-ext").Split(".", parts);
    if (parts.size() == 6)
    {
    ...
    if (!bQuiet)
        SetApplicationSetting("GninE-version-ext", SString("%d.%d.%d-%d.%05d.%c.%03d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR, GninESA_VERSION_MAINTENANCE,
                                                         GninESA_VERSION_TYPE, GninESA_VERSION_BUILD, usNetRev == 65535 ? '+' : usNetRev + '0', usNetRel));
}
...
{
    HANDLE hMutex = CreateMutex(NULL, FALSE, TEXT(GninE_GUID));
...
void CleanDownloadCache()
{
    ...
    cacheLocationList.push_back(PathJoin(GetGninEDataPath(), "upcache"));
    cacheLocationList.push_back(PathJoin(GetGninETempPath(), "upcache"));
    cacheLocationList.push_back(GetGninETempPath());
...
void MaybeShowCopySettingsDialog()
{
    ...
    SString       strCurrentConfig = PathJoin(GetGninESAPath(), "GninE", "config", "coreconfig.xml");
    if (FileExists(strCurrentConfig))
        return;

    // Check if coreconfig.xml from previous version is present
    SString strCurrentVersion = SString("%d.%d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR);
    SString strPreviousVersion = SString("%d.%d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR - 1);
    SString strPreviousPath = GetVersionRegistryValue(strPreviousVersion, "", "Last Run Location");
    if (strPreviousPath.empty())
        return;
    SString strPreviousConfig = PathJoin(strPreviousPath, "GninE", "config", "coreconfig.xml");
    if (GninESA_VERSION_MAJOR == 1 && GninESA_VERSION_MINOR == 5)
        strPreviousConfig = PathJoin(strPreviousPath, "GninE", "coreconfig.xml");
    if (!FileExists(strPreviousConfig))
        return;

    HideSplash();            // Hide standard GninE splash
    ...
    SString strCurrentNewsDir = PathJoin(GetGninEDataPath(), "news");
...
void BsodDetectionPreLaunch()
{
    ...
    if (strMinidumpTime > strLastMinidumpTime)
    {
        ...
        if (strMinidumpTime > strGameBeginTime && !strGameBeginTime.empty() && iBsodDetectSkip == 0)
        {
            // Ask user to confirm
            int iResponse =
                MessageBoxUTF8(NULL, _("Did your computer restart when playing GninE:SA?"), "GninE: San Andreas", MB_YESNO | MB_ICONQUESTION | MB_TOPMOST);
            if (iResponse == IDYES)
            {
