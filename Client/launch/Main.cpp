...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    {
        BrowseToSolution("launch-xpsp3-check", ASK_GO_ONLINE, "This version of GninE requires Windows XP SP3 or later");
        return 1;
    }
    ...
    SString strGninESAPath = PathJoin(GetLaunchPath(), "GninE");
    SString strLoaderDllPathFilename = PathJoin(strGninESAPath, strLoaderDllFilename);
    ...
    if (!hModule)
    {
        // Retry using GninE current directory
        SetCurrentDirectoryW(FromUTF8(strGninESAPath));
        hModule = LoadLibraryW(FromUTF8(strLoaderDllPathFilename));
        dwLoadLibraryError = GetLastError();
        if (hModule)
        {
            AddReportLog(5712, SString("LoadLibrary '%s' succeeded on change to directory '%s'", *strLoaderDllFilename, *strGninESAPath));
        }
    }
