...
{
    // Check if near any GninE objects
    ...
    if (!bNearObject)
    {
        // If not near any GninE objects, then don't bother waiting
    ...
    bool                  bGninEObjLimit = pObjectManager->IsObjectLimitReached();
    bool                  bHasModel = GetModelInfo() != NULL;
    #ifndef ASYNC_LOADING_DEBUG_OUTPUTA
    bool bGninELoaded = pObjectManager->ObjectsAroundPointLoaded(vecPosition, fUseRadius, m_usDimension);
    #else
    SString strAround;
    bool    bGninELoaded = pObjectManager->ObjectsAroundPointLoaded(vecPosition, fUseRadius, m_usDimension, &strAround);
    #endif

    #ifdef ASYNC_LOADING_DEBUG_OUTPUTA
    SString status = SString(
        "%2.2f,%2.2f,%2.2f  bASync:%d   bHasModel:%d   bGninELoaded:%d   bGninEObjLimit:%d   m_fGroundCheckTolerance:%2.2f   m_fObjectsAroundTolerance:%2.2f  "
        "fUseRadius:%2.1f",
        vecPosition.fX, vecPosition.fY, vecPosition.fZ, bASync, bHasModel, bGninELoaded, bGninEObjLimit, m_fGroundCheckTolerance, m_fObjectsAroundTolerance,
        fUseRadius);
    #endif

    // See if ground is ready
    if ((!bHasModel || !bGninELoaded) && m_fObjectsAroundTolerance < 1.f)
    {
        m_fGroundCheckTolerance = 0.f;
    ...
    else
    {
        ...
        if (m_fGroundCheckTolerance > 0.03f /*&& !bGninEObjLimit*/ && !bASync)
            SetFrozenWaitingForGroundToLoad(false, true);
    }
...
bool CClientVehicle::DoesNeedToWaitForGroundToLoad()
{
    ...
    // Check for GninE objects around our position
    CVector vecPosition;
    GetPosition(vecPosition);
    CClientObjectManager* pObjectManager = g_pClientGame->GetObjectManager();
