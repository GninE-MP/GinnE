...
void CPacketHandler::Packet_ServerDisconnected(NetBitStreamInterface& bitStream)
{
    ...
    if (bShowMessageBox)
    {
        ...
        std::vector<const char*> snippetList = {
            ...
            {_td("Remove D3D9.DLL from your GTA install directory and restart GninE")},
            ...
            {_td("Ensure no other program is modifying GninE:SA")},
        };
...
void CPacketHandler::Packet_PartialPacketInfo(NetBitStreamInterface& bitStream)
{
    ...
    BYTE          byteGninEPacketID;

    if (bitStream.Read(partCount) && bitStream.Read(partTotal) && bitStream.Read(partLength) && bitStream.Read(byteGninEPacketID))
    {
        if (byteGninEPacketID == RAKNET_PACKET_COUNT + PACKET_ID_ENTITY_ADD)
        {
            if (g_pClientGame)
            {
