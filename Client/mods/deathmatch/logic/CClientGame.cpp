...
CClientGame::CClientGame(bool bLocalPlay) : m_ServerInfo(new CServerInfo())
{
    ...
    */
    ...
    m_pScriptDebugging->SetLogfile(CalcGninESAPath("GninE\\logs\\clientscript.log"), 3);
...
#if defined(GninE_DEBUG) || defined(GninE_BETA)
...
bool CClientGame::StartGame(const char* szNick, const char* szPassword, eServerType Type)
{
    ...
    if (g_pNet->IsConnected() || m_bIsPlayingBack)
    {
        ...
        if (pBitStream)
        {
            ...
            pBitStream->Write(static_cast<unsigned short>(GninE_DM_NETCODE_VERSION));
            pBitStream->Write(static_cast<unsigned short>(GninE_DM_VERSION));
            pBitStream->Write(static_cast<unsigned short>(GninE_DM_BITSTREAM_VERSION));

            SString strPlayerVersion("%d.%d.%d-%d.%05d.%d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR, GninESA_VERSION_MAINTENANCE, GninESA_VERSION_TYPE,
                                     GninESA_VERSION_BUILD, g_pNet->GetNetRev());
...
void CClientGame::DoPulses()
{
    ...
    if (m_bWaitingForLocalConnect)
    {
        ...
        if (g_pNet->IsConnected())
        {
            ...
            g_pNet->SetServerBitStreamVersion(static_cast<unsigned short>(GninE_DM_BITSTREAM_VERSION));
    ...
    if (!m_bWaitingForLocalConnect)
    {
        ...
        if (m_Status == CClientGame::STATUS_JOINING)
        {
            ...
            {
                g_pCore->ShowNetErrorMessageBox(_("Error") + _E("CD19"), _("GninE Client verification failed!"));
                g_pCore->GetModManager()->RequestUnload();
            }
        }
    }
...
void CClientGame::AddBuiltInEvents()
{
    ...
    m_Events.AddEvent("onClientGninEFocusChange", "focused", NULL, false);
...
void CClientGame::IdleHandler()
{
    ...
    if (g_pCore->IsWindowMinimized())
    {
        if (!m_bWasMinimized)
        {
            ...
            if (bMuteAll || g_pCore->GetCVars()->GetValue<bool>("mute_GninE_when_minimized"))
...
bool CClientGame::ObjectBreakHandler(CObjectSAInterface* pObjectInterface, CEntitySAInterface* pAttackerInterface)
{
    if (pObjectInterface)
    {
        ...
        if (pObjectEntity)
        {
            ...
            // Apply to GninE's "internal storage", too
...
void CClientGame::OnWindowFocusChange(bool state)
{
    ...
    m_pRootEntity->CallEvent("onClientGninEFocusChange", Arguments, false);
}
