...
#define GninE_SERVERID_LOOKUP_DIR       "priv"
#define GninE_SERVERID_LOOKUP_XML       "priv/server-ids.xml"
...
CServerIdManagerImpl::CServerIdManagerImpl()
{
    ...
    m_strServerIdLookupBaseDir = PathJoin(g_pClientGame->GetFileCacheRoot(), GninE_SERVERID_LOOKUP_DIR);
...
bool CServerIdManagerImpl::LoadServerIdMap()
{
    ...
    CXMLFile* pConfigFile = g_pCore->GetXML()->CreateXML(PathJoin(g_pClientGame->GetFileCacheRoot(), GninE_SERVERID_LOOKUP_XML));
    if (!pConfigFile)
        return false;
    pConfigFile->Parse();
