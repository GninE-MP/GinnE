...
CClientObjectManager::CClientObjectManager(CClientManager* pManager)
{
    ...
    m_uiMaxStreamedInCount = MAX_OBJECTS_GninE / 2;
    m_uiMaxLowLodStreamedInCount = MAX_OBJECTS_GninE - m_uiMaxStreamedInCount;
}
...
{
    if (m_uiStreamedInCount + m_uiLowLodStreamedInCount >= MAX_OBJECTS_GninE)
        return true;

    // If we've run out of either of these limit, don't allow more objects
    if (m_iEntryInfoNodeEntries >= MAX_ENTRY_INFO_NODES_GninE || m_iPointerNodeSingleLinkEntries >= MAX_POINTER_SINGLE_LINKS_GninE ||
        m_iPointerNodeDoubleLinkEntries >= MAX_POINTER_DOUBLE_LINKS_GninE)
    {
        if (!m_bDoneLimitWarning)
        {
            ...
            SString strMessage(
                ...
                m_iEntryInfoNodeEntries, MAX_ENTRY_INFO_NODES_GninE, m_iPointerNodeSingleLinkEntries, MAX_POINTER_SINGLE_LINKS_GninE,
                m_iPointerNodeDoubleLinkEntries, MAX_POINTER_DOUBLE_LINKS_GninE);
    ...
    return g_pGame->GetPools()->GetObjectCount() >= MAX_OBJECTS_GninE;
}
