...
bool CStaticFunctionDefinitions::GetPedAnalogControlState(CClientPed& Ped, const char* szControl, float& fState, bool bRawInput)
{
    if (Ped.GetType() == CCLIENTPLAYER)
    {
        ...
        if (bRawInput)
            cs = Ped.m_rawControllerState;            // use the raw controller values without GninE glitch fixes modifying our raw inputs
        else
            Ped.GetControllerState(cs);
...
bool CStaticFunctionDefinitions::GetAnalogControlState(const char* szControl, float& fState, bool bRawInput)
{
    ...
    if (bRawInput)
        cs = pLocalPlayer->m_rawControllerState;            // use the raw controller values without GninE glitch fixes modifying our raw inputs
    else
        pLocalPlayer->GetControllerState(cs);
...
bool CStaticFunctionDefinitions::ToggleAllControls(bool bGTAControls, bool bGninEControls, bool bEnabled)
{
    CKeyBindsInterface* pKeyBinds = g_pCore->GetKeyBinds();
    pKeyBinds->SetAllControlsEnabled(bGTAControls, bGninEControls, bEnabled);
...
{
    return GninE_DM_NETCODE_VERSION;
}

const char* CStaticFunctionDefinitions::GetOperatingSystemName()
{
    return GninE_OS_STRING;
}

const char* CStaticFunctionDefinitions::GetVersionBuildTag()
{
    return GninE_DM_BUILDTAG_LONG;
}

SString CStaticFunctionDefinitions::GetVersionSortable()
{
    unsigned short usNetRev = g_pCore->GetNetwork()->GetNetRev();
    return SString("%d.%d.%d-%d.%05d.%d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR, GninESA_VERSION_MAINTENANCE, GninESA_VERSION_TYPE, GninESA_VERSION_BUILD,
                   usNetRev);
}
