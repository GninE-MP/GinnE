...
{
    unsigned char ucGTAControls, ucGninEControls, ucEnabled;
    if (bitStream.Read(ucGTAControls) && bitStream.Read(ucGninEControls) && bitStream.Read(ucEnabled))
    {
        g_pCore->GetKeyBinds()->SetAllControlsEnabled(ucGTAControls == 1, ucGninEControls == 1, ucEnabled == 1);
    }
}
