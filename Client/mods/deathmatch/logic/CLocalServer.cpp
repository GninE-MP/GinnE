...
CLocalServer::CLocalServer(const char* szConfig)
{
    ...
    m_pEditName = reinterpret_cast<CGUIEdit*>(m_pGUI->CreateEdit(m_pTabGeneral, "Default GninE Server"));
...
bool CLocalServer::Load()
{
    ...
    SString strServerPath = CalcGninESAPath(PathJoin("server", "mods", "deathmatch"));
...
bool CLocalServer::Save()
{
    ...
    {
        StoreConfigValue("servername", (m_pEditName->GetText().length() > 0) ? m_pEditName->GetText().c_str() : "GninE Local Server");
