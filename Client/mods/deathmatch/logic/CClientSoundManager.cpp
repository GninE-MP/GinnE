...
CClientSoundManager::CClientSoundManager(CClientManager* pClientManager)
{
    ...
    m_strUserAgent = SString("GninE:SA Server %s - See http://GninEsa.com/agent/", g_pNet->GetConnectedServer(true));
...
{
    // set our GninE sound volume if GninEvolume or mastervolume cvar changed
    float fValue = 0.0f;
    ...
    if (!m_bMinimizeMuted)
    {
        ...
        if (g_pCore->GetCVars()->Get("GninEvolume", fValue))
        {
