...
{
    bool             bEnabled = false;
    bool             bGTAControls = true, bGninEControls = true;
    CScriptArgReader argStream(luaVM);
    argStream.ReadBool(bEnabled);
    argStream.ReadBool(bGTAControls, true);
    argStream.ReadBool(bGninEControls, true);

    if (!argStream.HasErrors())
    {
        if (CStaticFunctionDefinitions::ToggleAllControls(bGTAControls, bGninEControls, bEnabled))
        {
            lua_pushboolean(luaVM, true);
            return 1;
        }
