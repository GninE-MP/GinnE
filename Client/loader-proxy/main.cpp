...
#ifdef GninE_DEBUG
constexpr std::wstring_view GninE_EXE_NAME = L"GninE_d.exe";
constexpr std::wstring_view CORE_DLL_NAME = L"core_d.dll";
constexpr std::wstring_view NETC_DLL_NAME = L"netc_d.dll";
#else
constexpr std::wstring_view GninE_EXE_NAME = L"GninE.exe";
...
void ApplyDirectoryInformation(HMODULE library, const std::wstring& GninEDirectory, const std::wstring& gtaDirectory);
...
VOID OnGameLaunch()
{
    ...
    // GninE:SA launches GTA:SA process with the GTA:SA installation directory as the current directory.
    ...
    // GninE:SA must be the parent launcher process in every case.
    ...
    // Check if the name of the launcher process matches GninE.
    const std::wstring launcherName = launcherPath.filename().wstring();

    if (!IEqual(GninE_EXE_NAME, launcherName))
    {
    ...
    // Check if the GninE subdirectory exists.
    const fs::path GninERootDirectory = launcherPath.parent_path();
    const fs::path GninEDirectory = GninERootDirectory / "GninE";

    if (!fs::is_directory(GninEDirectory, ec))
    {
        AddLaunchLog("GninE subdirectory not found (%d): %s", ec.value(), ec.message().c_str());
        std::wstring message = L"GninE directory does not exist or user is unauthorized:\n" + GninEDirectory.wstring();
        DisplayErrorMessageBox(MakeMissingFilesError(AppendSystemError(message, ec.value())), L"CL55");
        return;
    }

    // Check if the netc.dll library exists in the GninE subdirectory.
    const fs::path netcPath = GninEDirectory / NETC_DLL_NAME;
    ...
    // Check if the core.dll library exists in the GninE subdirectory.
    const fs::path corePath = GninEDirectory / CORE_DLL_NAME;
    ...
    // Patch the winmm.dll imports we've taken over with our GninEsa.dll library back to the functions from the winmm.dll library.
    ...
    SetDllDirectoryW(GninEDirectory.wstring().c_str());
    ...
    ApplyDirectoryInformation(g_netc, GninERootDirectory.wstring(), gtaDirectory.wstring());
    ...
    if (!g_core)
    {
        ...
        DisplayErrorMessageBox(
            ...
            L"\n\n"
            L"Please ensure that your GninE:SA build was successful and that "
        ...
        DisplayErrorMessageBox(
            ...
            L"\n\n"
            L"Please ensure that your GninE:SA installation is complete "
    ...
    ApplyDirectoryInformation(g_core, GninERootDirectory.wstring(), gtaDirectory.wstring());
...
{
    std::wstring caption = L"GninE: San Andreas  [" + errorCode + L"]   (CTRL+C to copy)";
    std::wstring extended = message;
    extended +=
        L"\n\nIf you are unable to resolve this issue on your own, "
        L"please get in contact with our support on www.gnine.com";
...
/**
 * @brief Replaces fake GninEsa.dll imports with functions retrieved from the winmm.dll library.
 */
auto PatchWinmmImports() -> int
{
    auto base = reinterpret_cast<std::byte*>(g_exe);
    ...
    // Iterate through the import descriptors and find the winmm.dll entry, which we renamed to GninEsa.dll.
    for (; descriptor->Name; ++descriptor)
    {
        auto name = reinterpret_cast<const char*>(base + descriptor->Name);

        if (stricmp("GninEsa.dll", name))
            continue;
 ...
 * @param GninEDirectory Path to the GninE:SA root directory
 * @param gtaDirectory Path to the GTA:SA root directory
 */
void ApplyDirectoryInformation(HMODULE library, const std::wstring& GninEDirectory, const std::wstring& gtaDirectory)
{
    // Set the path to the GninE directory.
    void (*SetGninEDirectory)(const wchar_t*, size_t) = reinterpret_cast<decltype(SetGninEDirectory)>(GetProcAddress(library, "SetGninEDirectory"));

    if (SetGninEDirectory)
    {
        SetGninEDirectory(GninEDirectory.c_str(), GninEDirectory.size());
    }
...
/**
 * @brief Composes an error message with the plea to launch GninE properly.
 * @param message Message to append to
 */
auto MakeLauncherError(std::wstring message) -> std::wstring
{
    ...
    message +=
        L"Please launch the game through the GninE launcher executable. "
        L"You can find the launcher either on your desktop or in the GninE:SA installation directory.";
...
auto MakeMissingFilesError(std::wstring message) -> std::wstring
{
    ...
    message +=
        L"Please ensure that both your GninE:SA and Windows installation are not missing files "
