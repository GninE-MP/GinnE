...
-- Config variables
local BIN_DIR = "Bin"
local DATA_DIR = "Shared/data/GninE San Andreas"
...
newaction {
	...
	execute = function()
		...
		local success, message = os.copydir("Server/mods/deathmatch", BIN_DIR.."/server/mods/deathmatch", "GninEserver.conf.template", false, true)
		if not success then
			errormsg("ERROR: Couldn't copy server config files", "\n"..message)
			os.exit(1)
			return
		...
		if os.host() == "windows" then
			...
			success = success and http.download_print_errors(NETC_PATH_WIN, BIN_DIR.."/GninE/netc.dll")
			...
			if not os.copyfile(BIN_DIR.."/GninE/netc.dll", BIN_DIR.."/GninE/netc_d.dll") then
