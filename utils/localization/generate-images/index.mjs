...
/**
 * Step 2
 * Copy GninE/locale folder over here
 */
import fse from 'fs-extra';

try {
	fse.copySync('../../../Shared/data/GninE San Andreas/GninE/locale', 'locale', { overwrite: true });
} catch (err) {
	console.error(err);
...
await (async () => {
    ...
    await Promise.allSettled(locales.map(async (locale) => {
        ...
        await Promise.allSettled(pngs.map(async (filename) => {
            ...
            return fs.writeFile(`../../../Shared/data/GninE San Andreas/GninE/locale/${locale}/${filename}`, (await image.encodedWith.oxipng).binary);
        }));
