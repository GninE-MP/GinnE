...
; Games explorer: With each new X.X, update this GUID and the file at GninE10\launch\NEU\GninE.gdf.xml
...
!ifndef FILES_ROOT
    ...
    !define INSTALL_OUTPUT "GninEsa-${0.0.0}-unstable-00000-0-000-nsis.exe"
...
!define PRODUCT_DIR_REGKEY "Software\Microsoft\Windows\CurrentVersion\App Paths\GninE ${0.0}.exe"
...
!define MUI_HEADERIMAGE_BITMAP "GninE_install_header.bmp"
!define MUI_HEADERIMAGE_BITMAP_RTL "GninE_install_header_rtl.bmp"
...
!define MUI_LANGDLL_REGISTRY_KEY "SOFTWARE\GninE: San Andreas All\${0.0}"
...
!ifndef VI_PRODUCT_VERSION
    ...
    !endif
    !define VI_PRODUCT_NAME "GninE San Andreas"
    !define VI_COMPANY_NAME "GninE"
    !define /date DATE_YEAR "%Y"
    !define VI_LEGAL_COPYRIGHT "(C) 2003 - ${DATE_YEAR} GninE"
    !ifndef LIGHTBUILD
        !define VI_FILE_DESCRIPTION "GninE Full Installer"
    !else
        !define VI_FILE_DESCRIPTION "GninE Nightly Installer"
    !endif
!endif
...
LangString	GET_XPVISTA_PLEASE	${LANG_ENGLISH} "The version of GninE:SA you've downloaded does not support Windows XP or Vista.  Please download an alternative version from www.GninEsa.com."
LangString	GET_WIN81_PLEASE	${LANG_ENGLISH} "The version of GninE:SA you've downloaded does not support Windows 7, 8 or 8.1.  Please download an alternative version from www.GninEsa.com."
LangString  GET_MASTER_PLEASE	${LANG_ENGLISH} "The version of GninE:SA you've downloaded is designed for old versions of Windows.  Please download an alternative version from www.GninEsa.com."
...
LangString  DIRECTORY_Text_Top  ${LANG_ENGLISH} "Please select your Grand Theft Auto: San Andreas folder.$\n$\nYou MUST have Grand Theft Auto: San Andreas 1.0 installed to use GninE:SA, it does not support any other versions.$\n$\nClick Install to begin installing."

; Language files
LangString  DESC_Section10          ${LANG_ENGLISH} "Create a Start Menu group for installed applications"
LangString  DESC_Section11          ${LANG_ENGLISH} "Create a Desktop Shortcut for the GninE:SA Client."
LangString  DESC_Section12          ${LANG_ENGLISH} "Register GninEsa:// protocol for browser clickable-ness."
...
LangString  DESC_Section1           ${LANG_ENGLISH} "The core components required to run GninE."
LangString  DESC_Section2           ${LANG_ENGLISH} "The GninE:SA modification, allowing you to play online."
;LangString DESC_Section3           ${LANG_ENGLISH} "The GninE:Editor for GninE:SA, allowing you to create and edit maps."
;LangString DESC_SectionGroupMods   ${LANG_ENGLISH} "Modifications for GninE. Without at least one of these, you cannot play GninE."
LangString  DESC_SectionGroupServer  ${LANG_ENGLISH}    "The GninE Server. This allows you to host games from your computer. This requires a fast internet connection."
LangString  DESC_Section4           ${LANG_ENGLISH} "The GninE server. This is a required component."
LangString  DESC_Section5           ${LANG_ENGLISH} "The GninE:SA modification for the server."
...
LangString  DESC_Section8           ${LANG_ENGLISH} "The GninE:SA 1.0 Map Editor.  This can be used to create your very own maps for use in gamemodes for GninE."
LangString  DESC_Section9           ${LANG_ENGLISH} "This is the SDK for creating binary modules for the GninE server. Only install if you have a good understanding of C++!"
;LangString DESC_Blank          ${LANG_ENGLISH} ""
LangString  DESC_SectionGroupDev        ${LANG_ENGLISH} "Development code and tools that aid in the creation of mods for GninE"
LangString  DESC_SectionGroupClient  ${LANG_ENGLISH}    "The client is the program you run to play on a GninE server"
...
Function .onInit
    ...
    ${If} ${AtMostWinVista}
        ...
        ExecShell "open" "https://gnine.com"
        Quit
    ${ElseIf} ${AtMostWin8.1}
    ...
    ${LogSetFileName} "$APPDATA\GninE San Andreas All\Common\Installer" "nsis.log"
    ...
    strcpy $DEFAULT_INSTDIR "$PROGRAMFILES\GninE San Andreas ${0.0}"
    ...
    ${Else}
        ...
        Pop $1
        ${If} $0 == "overwrite"
            # Ignore last used if it contains different major GninE version
            StrCpy $WhichRadio "default"
            StrCpy $ShowLastUsed "0"
        ${Else}
    ...
    ReadRegStr $2 HKLM "SOFTWARE\GninE: San Andreas All\Common" "GTA:SA Path"
    ${If} $2 == ""
        ReadRegStr $2 HKCU "SOFTWARE\GninE: San Andreas" "GTA:SA Path"
    ${EndIf}
    ${If} $2 == ""
        ReadRegStr $2 HKLM "SOFTWARE\Rockstar Games\GTA San Andreas\Installation" "ExePath"
    ${EndIf}
    ${If} $2 == ""
        ReadRegStr $2 HKLM "SOFTWARE\GninE: San Andreas" "GTA:SA Path"
    ${EndIf}
    ${If} $2 == ""
    ...
    ${EndIf}
    ${If} $2 == ""
        ReadRegStr $2 HKCU "SOFTWARE\GninE: San Andreas ${0.0}" "GTA:SA Path Backup"
    ${EndIf}
...
Function .onInstSuccess
    ...
    WriteRegStr HKLM "SOFTWARE\GninE: San Andreas All\Common" "GTA:SA Path" $GTA_DIR
	...
	Call GninEInitFileNamesAndPaths
    ; Start menu items
    ${If} $CreateSMShortcuts == 1
		${IfNot} ${FileExists} "$StartMenuGninEFolderPath\*.*"
			CreateDirectory $StartMenuGninEFolderPath
		${EndIf}
		# Either update or create Client shortcut
		${If} ${FileExists} $StartMenuClientShortcutPath
			Push $ClientExePath
			Push $StartMenuClientShortcutPath
			Call GninEUpdateShortсutTarget
		${Else}
			Push $ClientExePath
			Push $ClientExePath
			Push $StartMenuClientShortcutPath
			Call GninECreateShortсut
		${EndIf}
    ...
    ${If} $RegisterProtocol == 1
        ...
        WriteRegStr HKCR "GninEsa" "" "URL:GninE San Andreas Protocol"
        WriteRegStr HKCR "GninEsa" "URL Protocol" ""
        WriteRegStr HKCR "GninEsa\DefaultIcon" "" "$INSTDIR\GninE.exe"
        WriteRegStr HKCR "GninEsa\shell\open\command" "" '"$INSTDIR\GninE.exe"%1'
    ${EndIf}
...
LangString INST_PROTOCOL            ${LANG_ENGLISH} "Register GninEsa:// protocol"
...
SectionGroup /e "$(INST_SEC_CLIENT)" SECGCLIENT
    Section "$(INST_SEC_CORE)" SEC01
        ...
        CreateDirectory "$APPDATA\GninE San Andreas All\Common"
        CreateDirectory "$APPDATA\GninE San Andreas All\${0.0}"
        ...
        # Make the directory "$APPDATA\GninE San Andreas All" read write accessible by all users
        # Make the directory "$GTA_DIR" read write accessible by all users

        ${If} ${AtLeastWinVista}
            DetailPrint "$(INFO_UPDATE_PERMISSIONS)"

            # Fix permissions for GninE install directory
            ...
            ${Else}
                ...
                FastPerms::FullAccessPlox "$INSTDIR\GninE" "($PermissionsGroup)"
            ...
            ${EndIf}
            ${LogText} "FullAccess $APPDATA\GninE San Andreas All"
            FastPerms::FullAccessPlox "$APPDATA\GninE San Andreas All" "($PermissionsGroup)"

            # Remove GninE virtual store
            StrCpy $0 $INSTDIR
        ...
        Delete "$INSTDIR\GninE\winmm.dll"
        #############################################################
        ...
        AccessControl::GrantOnRegKey HKLM "SOFTWARE\GninE: San Andreas All" "($PermissionsGroup)" "FullAccess"

        SetOutPath "$INSTDIR\GninE"
        File "${FILES_ROOT}\GninE\cgui.dll"
        File "${FILES_ROOT}\GninE\core.dll"
        File "${FILES_ROOT}\GninE\xmll.dll"
        File "${FILES_ROOT}\GninE\game_sa.dll"
        File "${FILES_ROOT}\GninE\multiplayer_sa.dll"
        File "${FILES_ROOT}\GninE\netc.dll"
        File "${FILES_ROOT}\GninE\loader.dll"
        File "${FILES_ROOT}\GninE\GninEsa.dll"
        File "${FILES_ROOT}\GninE\pthread.dll"
        File "${FILES_ROOT}\GninE\cefweb.dll"
        File "${FILES_ROOT}\GninE\libwow64.dll"
        File "${FILES_ROOT}\GninE\wow64_helper.exe"

        File "${FILES_ROOT}\GninE\bass.dll"
        File "${FILES_ROOT}\GninE\bass_aac.dll"
        File "${FILES_ROOT}\GninE\bass_ac3.dll"
        File "${FILES_ROOT}\GninE\bass_fx.dll"
        File "${FILES_ROOT}\GninE\bassflac.dll"
        File "${FILES_ROOT}\GninE\bassmidi.dll"
        File "${FILES_ROOT}\GninE\bassmix.dll"
        File "${FILES_ROOT}\GninE\bassopus.dll"
        File "${FILES_ROOT}\GninE\basswebm.dll"
        File "${FILES_ROOT}\GninE\basswma.dll"
        File "${FILES_ROOT}\GninE\tags.dll"

        SetOutPath "$INSTDIR\GninE"
        File "${FILES_ROOT}\GninE\chrome_elf.dll"
        File "${FILES_ROOT}\GninE\libcef.dll"
        File "${FILES_ROOT}\GninE\icudtl.dat"
        File "${FILES_ROOT}\GninE\libEGL.dll"
        File "${FILES_ROOT}\GninE\libGLESv2.dll"
        File "${FILES_ROOT}\GninE\vk_swiftshader.dll"
        File "${FILES_ROOT}\GninE\vulkan-1.dll"
        File "${FILES_ROOT}\GninE\snapshot_blob.bin"
        File "${FILES_ROOT}\GninE\v8_context_snapshot.bin"

        File "${FILES_ROOT}\GninE\XInput9_1_0_GninE.dll"
        File "${FILES_ROOT}\GninE\xinput1_3_GninE.dll"
        File "${FILES_ROOT}\GninE\d3dcompiler_43.dll"
        File "${FILES_ROOT}\GninE\d3dcompiler_47.dll"

        SetOutPath "$INSTDIR\GninE\CEF"
        File "${FILES_ROOT}\GninE\CEF\CEFLauncher.exe"
        File "${FILES_ROOT}\GninE\CEF\CEFLauncher_DLL.dll"


	# Added as per https://bitbucket.org/chromiumembedded/cef/commits/8424f166ccef
        File "${FILES_ROOT}\GninE\CEF\chrome_100_percent.pak"
        File "${FILES_ROOT}\GninE\CEF\chrome_200_percent.pak"
        File "${FILES_ROOT}\GninE\CEF\resources.pak"

	# Clarification for the below 4 deprecated files: https://bitbucket.org/chromiumembedded/cef/commits/8424f166ccef
        #File "${FILES_ROOT}\GninE\CEF\cef.pak"
        #File "${FILES_ROOT}\GninE\CEF\cef_100_percent.pak"
        #File "${FILES_ROOT}\GninE\CEF\cef_200_percent.pak"
        #File "${FILES_ROOT}\GninE\CEF\devtools_resources.pak"
		
	# Below file was included in the deprecation referenced above, but already disabled in GninE beforehand
        #File "${FILES_ROOT}\GninE\CEF\cef_extensions.pak"

        SetOutPath "$INSTDIR\GninE\CEF\locales"
        File "${FILES_ROOT}\GninE\CEF\locales\en-US.pak"


        ${If} "$(LANGUAGE_CODE)" != ""
            # Write our language to registry
            WriteRegStr HKLM "SOFTWARE\GninE: San Andreas All\${0.0}\Settings\general" "locale" "$(LANGUAGE_CODE)"
        ${EndIf}

        !ifndef LIGHTBUILD

            SetOutPath "$INSTDIR\GninE"
            File "${FILES_ROOT}\GninE\d3dx9_42.dll"
            File "${FILES_ROOT}\GninE\D3DCompiler_42.dll"
            File "${FILES_ROOT}\GninE\sa.dat"
            File "${FILES_ROOT}\GninE\XInput9_1_0_GninE.dll"
            File "${FILES_ROOT}\GninE\xinput1_3_GninE.dll"
            ...
            SetOutPath "$INSTDIR\GninE\data"
            File "${FILES_ROOT}\GninE\data\gta_sa_diff.dat"

            SetOutPath "$INSTDIR\GninE\config"
            File "${FILES_ROOT}\GninE\config\chatboxpresets.xml"
            ...
            File "${FILES_ROOT}\GninE\cgui\Falagard.xsd"
            File "${FILES_ROOT}\GninE\cgui\Font.xsd"
            File "${FILES_ROOT}\GninE\cgui\GUIScheme.xsd"
            File "${FILES_ROOT}\GninE\cgui\Imageset.xsd"
            File "${FILES_ROOT}\GninE\cgui\pricedown.ttf"
            File "${FILES_ROOT}\GninE\cgui\sabankgothic.ttf"
            File "${FILES_ROOT}\GninE\cgui\sagothic.ttf"
            File "${FILES_ROOT}\GninE\cgui\saheader.ttf"
            File "${FILES_ROOT}\GninE\cgui\sans.ttf"
            File "${FILES_ROOT}\GninE\cgui\unifont.ttf"

            SetOutPath "$INSTDIR\GninE\cgui\images"
            File "${FILES_ROOT}\GninE\cgui\images\*.png"
            File "${FILES_ROOT}\GninE\cgui\images\*.jpg"

            SetOutPath "$INSTDIR\GninE\cgui\images\radarset"
            File "${FILES_ROOT}\GninE\cgui\images\radarset\*.png"

            SetOutPath "$INSTDIR\GninE\cgui\images\transferset"
            File "${FILES_ROOT}\GninE\cgui\images\transferset\*.png"

            SetOutPath "$INSTDIR\GninE\cgui\images\serverbrowser"
            File "${FILES_ROOT}\GninE\cgui\images\serverbrowser\*.png"

        !endif

        SetOutPath "$INSTDIR\GninE\locale\"
        File /r "${FILES_ROOT}\GninE\locale\*.png"
        File /r "${FILES_ROOT}\GninE\locale\*.pot"

        SetOutPath "$INSTDIR"
        File "${FILES_ROOT}\GninE.exe"

        # Ensure exe file can be updated without admin
        AccessControl::GrantOnFile "$INSTDIR\GninE.exe" "($PermissionsGroup)" "FullAccess"

        ${If} $AddToGameExplorer == 1
            ${GameExplorer_UpdateGame} ${GUID}
            ${If} ${Errors}
                ${GameExplorer_AddGame} all "$INSTDIR\GninE.exe" "$INSTDIR" "$INSTDIR\GninE.exe" ${GUID}
                CreateDirectory $APPDATA\Microsoft\Windows\GameExplorer\${GUID}\SupportTasks\0
                CreateShortcut "$APPDATA\Microsoft\Windows\GameExplorer\$0\SupportTasks\0\Client Manual.lnk" \ "https://wiki.gnine.com/wiki/Client_Manual"
            ${EndIf}
        ${EndIf}
...
SectionGroup /e "$(INST_SEC_SERVER)" SECGSERVER
    Section "$(INST_SEC_CORE)" SEC04
        ...
        File "${FILES_ROOT}\GninE\xmll.dll"
        File "${SERVER_FILES_ROOT}\GninE Server.exe"
    ...
    Section "$(INST_SEC_GAME)" SEC05
        ...
        File "${SERVER_FILES_ROOT}\mods\deathmatch\GninEserver.conf"
...
Section -Post
    ...
    ;WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\GninE.exe"

    WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
    WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\Uninstall.exe"
    WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\GninE.exe"
...
$\r$\nIt is not necessary to uninstall before installing a new version of GninE:SA\
...
Section Uninstall
    ...
    preservemapsfolder:
        ...
        Delete "$INSTDIR\server\GninE Server.exe"
        ...
        Delete "$INSTDIR\server\GninE Server64.exe"
        ...
        Delete "$INSTDIR\GninE.exe"
        Delete "$INSTDIR\GninE.exe.dat"
        Delete "$INSTDIR\Uninstall.exe"

        Delete "$INSTDIR\mods\deathmatch\client.dll"
        Delete "$INSTDIR\mods\deathmatch\pcre3.dll"

        RmDir /r "$INSTDIR\GninE\cgui"
        RmDir /r "$INSTDIR\GninE\data"
        RmDir /r "$INSTDIR\GninE\CEF"
        RmDir /r "$INSTDIR\GninE\locale"
        Delete "$INSTDIR\GninE\*.dll"
        Delete "$INSTDIR\GninE\*.exe"
        Delete "$INSTDIR\GninE\*.dmp"
        Delete "$INSTDIR\GninE\*.log"
        Delete "$INSTDIR\GninE\*.dat"
        Delete "$INSTDIR\GninE\*.bin"

        RmDir /r "$APPDATA\GninE San Andreas All\${0.0}"
        ; Delete "$APPDATA\GninE San Andreas All" if "Common" is the only directory in it.
        ${RmDirWithSingleChildDir} "$APPDATA\GninE San Andreas All" "Common"

        DeleteRegKey ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}"
        DeleteRegKey HKLM "${PRODUCT_DIR_REGKEY}"
        DeleteRegKey HKLM "SOFTWARE\GninE: San Andreas ${0.0}"
        DeleteRegKey HKCU "SOFTWARE\GninE: San Andreas ${0.0}"
        DeleteRegKey HKLM "SOFTWARE\GninE: San Andreas All\${0.0}"
        ; Delete "SOFTWARE\GninE: San Andreas All" if "Common" is the only one left.
        ${RemoveRegistryGroupWithSingleKey} HKLM "SOFTWARE\GninE: San Andreas All" "Common"

        ReadRegStr $0 HKLM "Software\Classes\GninEsa\DefaultIcon" ""
        ${If} $0 == "$INSTDIR\GninE.exe"
            DeleteRegKey HKCR "GninEsa"
        ${EndIf}

        ${GameExplorer_RemoveGame} ${GUID}

        ; Delete client shortcuts
        Delete "$SMPROGRAMS\\GninE San Andreas ${0.0}\GninE San Andreas.lnk"
        Delete "$SMPROGRAMS\\GninE San Andreas ${0.0}\Uninstall GninE San Andreas.lnk"
        Delete "$DESKTOP\GninE San Andreas ${0.0}.lnk"

        ; Delete server shortcuts
        Delete "$SMPROGRAMS\\GninE San Andreas ${0.0}\GninE Server.lnk"
        Delete "$SMPROGRAMS\\GninE San Andreas ${0.0}\Uninstall GninE San Andreas Server.lnk"
        RmDir /r "$SMPROGRAMS\\GninE San Andreas ${0.0}"

        RmDir "$INSTDIR" ; fix for #3898

        DeleteRegValue HKCR "Local Settings\Software\Microsoft\Windows\Shell\MuiCache" "$INSTDIR\GninE.exe.FriendlyAppName"
        DeleteRegValue HKCR "Local Settings\Software\Microsoft\Windows\Shell\MuiCache" "$INSTDIR\GninE.exe.ApplicationCompany"
        DeleteRegValue HKCR "Local Settings\Software\Microsoft\Windows\Shell\MuiCache" "$INSTDIR\GninE\wow64_helper.exe.FriendlyAppName"
        DeleteRegValue HKCR "Local Settings\Software\Microsoft\Windows\Shell\MuiCache" "$INSTDIR\GninE\wow64_helper.exe.ApplicationCompany"
        DeleteRegValue HKCR "Local Settings\Software\Microsoft\Windows\Shell\MuiCache" "$INSTDIR\server\GninE Server.exe.FriendlyAppName"
        DeleteRegValue HKCR "Local Settings\Software\Microsoft\Windows\Shell\MuiCache" "$INSTDIR\server\GninE Server.exe.ApplicationCompany"
        DeleteRegValue HKCR "Local Settings\Software\Microsoft\Windows\Shell\MuiCache" "$INSTDIR\server\GninE Server64.exe.FriendlyAppName"
        DeleteRegValue HKCR "Local Settings\Software\Microsoft\Windows\Shell\MuiCache" "$INSTDIR\server\GninE Server64.exe.ApplicationCompany"
        DeleteRegValue HKCR "Local Settings\Software\Microsoft\Windows\Shell\MuiCache" "$APPDATA\GninE San Andreas All\${0.0}\GTA San Andreas\gta_sa.exe.FriendlyAppName"
        DeleteRegValue HKCU "Software\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Compatibility Assistant\Store" "$INSTDIR\GninE.exe"
        DeleteRegValue HKCU "Software\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Compatibility Assistant\Store" "$INSTDIR\server\GninE Server.exe"
        DeleteRegValue HKCU "Software\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Compatibility Assistant\Store" "$INSTDIR\server\GninE Server64.exe"
    ...
    loop:
        ...
        ${ReadRegStrMultiSz} ${HKEY_CURRENT_USER} "Software\Microsoft\Windows\CurrentVersion\UFH\SHC" $8 "$INSTDIR\GninE.exe" $9
        ${If} $9 != ""
            DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\UFH\SHC" $8
            Goto loop
        ${EndIf}

        ${ReadRegStrMultiSz} ${HKEY_CURRENT_USER} "Software\Microsoft\Windows\CurrentVersion\UFH\SHC" $8 "$INSTDIR\server\GninE Server.exe" $9
        ${If} $9 != ""
            DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\UFH\SHC" $8
            Goto loop
        ${EndIf}
        ...
        ${ReadRegStrMultiSz} ${HKEY_CURRENT_USER} "Software\Microsoft\Windows\CurrentVersion\UFH\SHC" $8 "$INSTDIR\GninE Server.exe" $9
        ${If} $9 != ""
            DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\UFH\SHC" $8
            Goto loop
        ${EndIf}
        ...
        SimpleFC::RemoveApplication "$INSTDIR\GninE.exe"
        SimpleFC::RemoveApplication "$INSTDIR\server\GninE Server.exe"
        SimpleFC::RemoveApplication "$INSTDIR\server\GninE Server64.exe"
        SimpleFC::RemoveApplication "$INSTDIR\Uninstall.exe"
        SimpleFC::RemoveApplication "$APPDATA\GninE San Andreas All\${0.0}\GTA San Andreas\gta_sa.exe"
...
Function ResizeSharedDialogContents
    ...
    StrCpy $ITEM_PARENT $HWNDPARENT
    StrCpy $ITEM_ID 1038    ; Static - "Choose which features of GninE:SA v1.0 you want to install."
    Call MoveDialogItem
    ...
    ${If} "$(LANGUAGE_RTL)" == "1"
        ...
        StrCpy $ITEM_PARENT $HWNDPARENT
        StrCpy $ITEM_ID 1046    ; Static - GninE_install_header_rtl.bmp
        Call MoveDialogItem
    ${EndIf}
...
Function RemoveVirtualStore
    ...
    ; Also remove VirtualStore\ProgramData\GninE San Andreas All
    StrCpy $3 "$LOCALAPPDATA\VirtualStore\ProgramData\GninE San Andreas All"
    ${If} ${FileExists} $3
        ${LogText} "Removing $3"
        RmDir /r "$3"
    ${Else}
...
LangString INST_GninE_CONFLICT ${LANG_ENGLISH}    "A different major version of GninE ($1) already exists at that path.$\n$\n\
            GninE is designed for major versions to be installed in different paths.$\n \
            Are you sure you want to overwrite GninE $1 at \
            $INSTDIR ?"
LangString INST_GTA_CONFLICT ${LANG_ENGLISH}    "GninE cannot be installed into the same directory as GTA:SA.$\n$\n\
...
Function "CustomDirectoryPageLeave"
    ...
    # Check if user is trying to install GninE into GTA directory
    Push $INSTDIR
    Call IsGtaDirectory
    Pop $0
    ${If} $0 == "gta"

        # Don't allow install into GTA directory unless GninE is already there
        Push $INSTDIR
        Call GetInstallType
        Pop $0
        Pop $1
    ...
    # Check if user is trying to install over a different major version of GninE
    Push $INSTDIR
    Call GetInstallType
    Pop $0
    Pop $1
    ${If} $0 == "overwrite"
        MessageBox MB_OKCANCEL|MB_ICONQUESTION|MB_TOPMOST|MB_SETFOREGROUND \
            "$(INST_GninE_CONFLICT)" \
            IDOK cont
            ${LogText} "-Function end - CustomDirectoryPageLeave (GninE_CONFLICT)"
            Abort
        cont:
    ${Endif}
...
Function CustomDirectoryPageSetDefaultButtonClick
    StrCpy $INSTDIR "$PROGRAMFILES\GninE San Andreas ${0.0}"
...
LangString INST_LOC_OW ${LANG_ENGLISH}  "Warning: A different major version of GninE ($1) already exists at that path."
...
Function DoServiceInstall
    ${If} $ServiceModified != 1
        ...
        GetDLLVersion "$INSTDIR\GninE\loader.dll" $R0 $R1
        IntOp $R5 $R1 & 0x0000FFFF ; $R5 now contains build
        ${If} $R5 > 4909
            Exec '"$INSTDIR\GninE.exe" /nolaunch /kdinstall'
...
Function un.DoServiceUninstall
    ${If} $ServiceModified != 2
        ...
        IntOp $R5 $R1 & 0x0000FFFF ; $R5 now contains build
        ${If} $R5 > 4909
            Exec '"$INSTDIR\GninE.exe" /nolaunch /kduninstall'
...
Function NetComposeURL
    ${If} $NetMsgURL == ""
        IfFileExists "$APPDATA\GninE San Andreas All" 0 skip
    ...
    ${Switch} $NetMirror
        ${Case} 0
            StrCpy $NetMsgURL "https://updatesa.gnine.com/sa/install/1/?x=0"
            ${Break}
        ${Default}
            StrCpy $NetMsgURL "https://updatesa.gnine.com/sa/install/1/?x=0"
            ${Break}
...
Function GninECreateShortсut
	...
	CreateShortCut $2 \
		...
		SW_SHOWNORMAL \
		"" \
		"Play GninE: San Andreas"
	${If} ${Errors}
		${LogText} "Error creating shortcut for EXE $2"
	${EndIf}
FunctionEnd

Function GninEUpdateShortсutTarget
...
Function GninEInitFileNamesAndPaths
	# Shortcuts names
	StrCpy $ClientShortcutName "GninE San Andreas"
	StrCpy $ServerShortcutName "GninE Server"
	StrCpy $UninstallShortcutName "Uninstall GninE San Andreas"
	# Shostcuts paths
	StrCpy $StartMenuGninEFolderPath "$SMPROGRAMS\GninE San Andreas ${0.0}"
	StrCpy $StartMenuClientShortcutPath "$StartMenuGninEFolderPath\$ClientShortcutName.lnk"
	StrCpy $StartMenuServerShortcutPath "$StartMenuGninEFolderPath\$ServerShortcutName.lnk"
	StrCpy $StartMenuUninstallShortcutPath "$StartMenuGninEFolderPath\$UninstallShortcutName.lnk"
	...
	# Exe names
	StrCpy $ClientExeName "GninE.exe"
	StrCpy $ServerExeName "GninE Server.exe"
