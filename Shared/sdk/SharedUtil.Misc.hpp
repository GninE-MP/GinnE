...
// #define PRODUCT_REGISTRY_PATH       "Software\\GninE: San Andreas All"       // HKLM
// #define PRODUCT_COMMON_DATA_DIR     "GninE San Andreas All"                               // C:\ProgramData
...
// Used in the Win32 Client only
//
SString SharedUtil::GetGninESABaseDir()
{
    ...
    if (strInstallRoot.empty())
    {
        ...
        if (strInstallRoot.empty())
        {
            ...
            if (strInstallRoot.empty())
            {
                MessageBoxUTF8(0, _("GninE has not been installed properly, please reinstall."), _("Error") + _E("U01"),
...
//
// Turns a relative GninESA path i.e. "GninE\file.dat"
// into an absolute GninESA path i.e. "C:\Program Files\GninE San Andreas\GninE\file.dat"
//
SString SharedUtil::CalcGninESAPath(const SString& strPath)
{
    return PathJoin(GetGninESABaseDir(), strPath);
}

//
// Returns true if current process is GTA (i.e not GninE process)
//
...
{
    return SStringX(GninE_DM_ASE_VERSION).Left(3);
}
...
//
// New layout:
//              HKLM Software\\GninE: San Andreas All\\Common    - For all versions
//              HKLM Software\\GninE: San Andreas All\\1.1       - For 1.1
//              HKLM Software\\GninE: San Andreas All\\1.2       - For 1.2
//
...
{
    // Encode into a string and set a registry key
    SString strVersion("%d.%d.%d-%d.%05d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR, GninESA_VERSION_MAINTENANCE, GninESA_VERSION_TYPE, GninESA_VERSION_BUILD);
...
const SString& SharedUtil::GetProductVersion()
{
    if (ms_strProductVersion.empty())
        ms_strProductVersion =
            SString("%d.%d.%d-%d.%05d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR, GninESA_VERSION_MAINTENANCE, GninESA_VERSION_TYPE, GninESA_VERSION_BUILD);
...
bool SharedUtil::ProcessPendingBrowseToSolution()
{
    ...
    SString strTitle("GninE: San Andreas %s   (CTRL+C to copy)", *strErrorCode);
    ...
    strQueryURL = strQueryURL.Replace("_VERSION_", GetApplicationSetting("GninE-version-ext"));
...
{
    SString strGninEBuild = GetApplicationSetting("GninE-version-ext").SplitRight("-");
    ...
    SString strResult = "[";
    if (strGninEBuild.length())
        strResult += strGninEBuild + " ";
...
void SharedUtil::AddReportLog(uint uiId, const SString& strText, uint uiAmountLimit)
{
    ...
    while (ms_ReportLineFilter.PopOutputLine(line))
    {
        ...
        SString strPathFilename = PathJoin(GetGninEDataPath(), "report.log");
...
{
    SString strReportFilename = PathJoin(GetGninEDataPath(), "report.log");
...
void WriteEvent(const char* szType, const SString& strText)
{
    ...
    SString strPathFilename = CalcGninESAPath(PathJoin("GninE", "logs", "logfile.txt"));
...
void SharedUtil::BeginEventLog()
{
    ...
    if (GetApplicationSettingInt("no-cycle-event-log") == 0)
    {
        ...
        CycleFile(CalcGninESAPath(PathJoin("GninE", "logs", "logfile.txt")), 1);
    }
...
#if defined(GninE_DM_EXPIRE_DAYS)
int SharedUtil::GetDaysUntilExpire()
{
    ...
    when.tm_year = BUILD_YEAR - 1900;
    when.tm_mon = BUILD_MONTH;
    when.tm_mday = BUILD_DAY + GninE_DM_EXPIRE_DAYS;
...
//
// For checking GninE library module versions
//
GninEEXPORT void GetLibMtaVersion(char* pBuffer, uint uiMaxSize)
{
    SString strVersion("%d.%d.%d-%d.%05d.%d"
#ifdef GninESA_VERSION_MAJOR
                       ,
                       GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR, GninESA_VERSION_MAINTENANCE, GninESA_VERSION_TYPE, GninESA_VERSION_BUILD
#else
                       ,
                       0, 0, 0, 0, 0
#endif
