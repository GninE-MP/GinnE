...
SString SharedUtil::GetGninEDataPath()
{
    return PathJoin(GetSystemCommonAppDataPath(), GetProductCommonDataDir(), GetMajorVersionString());
}

SString SharedUtil::GetGninEDataPathCommon()
{
    return PathJoin(GetSystemCommonAppDataPath(), GetProductCommonDataDir(), "Common");
}

SString SharedUtil::GetGninETempPath()
{
    return PathJoin(GetSystemTempPath(), "GninE" + GetMajorVersionString());
}
...
SString SharedUtil::GetDriveNameWithNotEnoughSpace(uint uiResourcesPathMinMB, uint uiDataPathMinMB)
{
    ...
    if (GetPathFreeSpaceMB(GetGninESABaseDir()) < uiResourcesPathMinMB)
        return GetPathDriveName(GetGninESABaseDir());
