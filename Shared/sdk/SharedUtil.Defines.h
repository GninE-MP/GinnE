...
//
#define GninE_STR_EXPAND(token) #token
#define GninE_STR(token) GninE_STR_EXPAND(token)
...
// As NDEBUG is not defined across most GninE projects, assert() will always be enabled
// Use dassert for debug build only assertations
#if defined(GninE_DEBUG) || defined(DEBUG) || defined(_DEBUG)
...
// Export function
#if defined _WIN32
    #define GninEEXPORT extern "C" __declspec(dllexport)
#else
    #define GninEEXPORT extern "C" __attribute__ ((visibility ("default")))
#endif
