...
// Define libraries
char szNetworkLibName[] = "net" GninE_LIB_SUFFIX GninE_LIB_EXTENSION;
char szXMLLibName[] = "xmll" GninE_LIB_SUFFIX GninE_LIB_EXTENSION;
...
int CServerImpl::Run(int iArgumentCount, char* szArguments[])
{
    ...
    // Welcome text
    if (!g_bSilent)
        Print("GninE:BLUE Server for GninE:SA\n\n");
    ...
    if (m_NetworkLibrary.Load(PathJoin(m_strServerPath, SERVER_BIN_PATH, szNetworkLibName)))
    {
        ...
        if (!pfnCheckCompatibility || !pfnCheckCompatibility(GninE_DM_SERVER_NET_MODULE_VERSION, (unsigned long*)GninESA_VERSION_TYPE))
        {
