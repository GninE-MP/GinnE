...
GninEEXPORT int Run(int iArgumentCount, char* szArguments[])
{
    if (iArgumentCount > 1)
    {
        if (strcmp(szArguments[1], "--version") == 0 || strcmp(szArguments[1], "-v") == 0)
        {
            printf(GninE_DM_FULL_STRING " v" GninE_DM_BUILDTAG_LONG "\n");
            return 1;
        }
    }
...
GninEEXPORT bool SendServerCommand(const char* szString)
{
