...
bool CDynamicLibrary::CheckMtaVersion(const char* szLibName)
{
    ...
    // define GninESA_SKIP_VERSION_CHECKS in "Shared/build_overrides.h" to skip version checks
#ifndef GninESA_SKIP_VERSION_CHECKS
    ...
    if (strVersionCore != strVersionLibrary)
    {
        ...
        if (GninESA_VERSION_BUILD == 0)
        {
            int iBuildLibrary = atoi(strVersionLibrary.SubStr(8));
            Print("\n(If this is a custom build,\n");
            Print(" 1. Set GninESA_VERSION_BUILD in version.h to %d\n", iBuildLibrary);
            Print(" 2. Rebuild)\n\n\n");
        }
        else
            Print("\n** REINSTALL GninE **\n\n");
