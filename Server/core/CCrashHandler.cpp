...
void CCrashHandler::Init(const SString& strInServerPath)
{
    ...
    SString strFilename("server_%s_%04d%02d%02d_%02d%02d.dmp", GninE_DM_BUILDTAG_LONG, tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min);
...
inline __attribute__((always_inline)) static void SaveBacktraceSummary()
{
    ...
    SString sFileName;
    sFileName.Format("server_%s_%04d%02d%02d_%02d%02d.log", GninE_DM_BUILDTYPE, tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday, tm->tm_hour, tm->tm_min);

    SString sContent;
    sContent += SString("GninE:SA Server v%s-r%d-%s crash report.\n", GninE_DM_VERSIONSTRING, GninESA_VERSION_BUILD, GninE_DM_BUILDTYPE);
...
void CCrashHandler::DumpMiniDump(_EXCEPTION_POINTERS* pException, CExceptionInformation* pExceptionInformation)
{
    ...
    if (hDll)
    {
        ...
        if (pDump)
        {
            ...
            SString strFilename("server_%s_%s_%08x_%x_%04d%02d%02d_%02d%02d.dmp", GninE_DM_BUILDTAG_LONG, strModuleName.c_str(),
            ...
            if (pFile)
            {
                ...
                SString strGninEVersionFull = SString("%s", GninE_DM_BUILDTAG_LONG);
