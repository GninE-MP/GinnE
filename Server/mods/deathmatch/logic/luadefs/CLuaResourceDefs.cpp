...
int CLuaResourceDefs::call(lua_State* luaVM)
{
    ...
    if (!argStream.HasErrors())
    {
        ...
        // call an GninE function in an external resource that's restricted and not
        // defined in ACL.
...
int CLuaResourceDefs::LoadString(lua_State* luaVM)
{
    ...
    if (!argStream.HasErrors())
    {
        ...
        {
            SString strMessage("argument 1 is invalid. Please re-compile at http://luac.GninEsa.com/", 0);
...
int CLuaResourceDefs::Load(lua_State* luaVM)
{
    ...
    if (!argStream.HasErrors())
    {
        ...
        {
            SString strMessage("argument 2 is invalid. Please re-compile at http://luac.GninEsa.com/", 0);
            argStream.SetCustomError(strMessage);
            cpBuffer = NULL;
        }
