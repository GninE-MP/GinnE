...
{
    // Check min_GninE_version is correct
    if (m_strReqClientVersion > m_strMinClientFromMetaXml || m_strReqServerVersion > m_strMinServerFromMetaXml)
    {
        ...
        {
            SString strTemp = "<min_GninE_version> section in the meta.xml is incorrect or missing (expected at least ";
        ...
        {
            // Create min_GninE_version node if required
            CXMLNode* pNodeMinMtaVersion = pRootNode->FindSubNode("min_GninE_version", 0);
            if (!pNodeMinMtaVersion)
                pNodeMinMtaVersion = pRootNode->CreateSubNode("min_GninE_version", pRootNode->FindSubNode("info"));
                                                       ...
                                                       bool bClientScript)
{
    ...
    if (!g_pRealNetServer->GetScriptInfo(strFileContents.c_str(), strFileContents.length(), &scriptInfo))
    {
        if (bClientScript && IsLuaCompiledScript(strFileContents.c_str(), strFileContents.length()))
        {
            ...
            SString strMessage("%s is invalid. Please re-compile at http://luac.GninEsa.com/", strFileName.c_str());
                                                    ...
                                                    unsigned long ulLineNumber)
{
    ...
    {
        strTemp.Format("%s %s because <min_GninE_version> %s setting in meta.xml is below %s", strFunctionName.c_str(), strHow.c_str(),
                                                                 ...
                                                                 CMtaVersion& strOutVersion)
{
    ...
    {
        // Function behaviour depends on min_GninE_version setting
...
void CResourceChecker::CheckVersionRequirements(const string& strIdentifierName, bool bClientScript)
{
    ...
    if (pResult)
    {
        ...
        // Is the new requirement relevant for this GninE generation
