...
static const SString g_GninEControls[] = {
    "chatbox",
    "voiceptt",
    "enter_passenger",
    "radar",
...
CPad::CPad(CPlayer* pPlayer)
{
    ...
    for (int i = 0; i < NUM_GninE_CONTROL_STATES; i++)
    {
        m_GninEEnabledControls[i] = true;
    }
...
bool CPad::IsControlEnabled(const char* szControl, bool& bEnabled)
{
    ...
    for (int i = 0; i < NUM_GninE_CONTROL_STATES; i++)
    {
        if (g_GninEControls[i] == szControl)
        {
            bEnabled = m_GninEEnabledControls[i];
            return true;
        }
    }
...
bool CPad::SetControlEnabled(const char* szControl, bool bEnabled)
{
    ...
    // Check if it is a GninE control
    for (int i = 0; i < NUM_GninE_CONTROL_STATES; i++)
    {
        if (g_GninEControls[i] == szControl)
        {
            m_GninEEnabledControls[i] = bEnabled;
            return true;
        }
    }
...
void CPad::SetAllGninEControlsEnabled(bool bEnabled)
{
    for (int i = 0; i < NUM_GninE_CONTROL_STATES; i++)
    {
        m_GninEEnabledControls[i] = bEnabled;
