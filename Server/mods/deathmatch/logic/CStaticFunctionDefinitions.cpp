...
bool CStaticFunctionDefinitions::ToggleAllControls(CPlayer* pPlayer, bool bGTAControls, bool bGninEControls, bool bEnabled)
{
    assert(pPlayer);
    ...
    if (bGninEControls)
        pPlayer->GetPad()->SetAllGninEControlsEnabled(bEnabled);

    CBitStream BitStream;
    BitStream.pBitStream->Write(static_cast<unsigned char>((bGTAControls) ? 1 : 0));
    BitStream.pBitStream->Write(static_cast<unsigned char>((bGninEControls) ? 1 : 0));
    BitStream.pBitStream->Write(static_cast<unsigned char>((bEnabled) ? 1 : 0));
...
{
    return SString("%d.%d.%d-%d.%05d.%d", GninESA_VERSION_MAJOR, GninESA_VERSION_MINOR, GninESA_VERSION_MAINTENANCE, GninESA_VERSION_TYPE, GninESA_VERSION_BUILD, 0);
}
