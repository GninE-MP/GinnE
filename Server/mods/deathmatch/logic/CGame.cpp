...
void CGame::GetTag(char* szInfoTag, int iInfoTag)
{
    ...
    SString strInfoTag("%c[%c%c%c] GninE: San Andreas %c:%c: %d/%d players %c:%c: %u resources", 132, 135, szProgress[ucProgress], 132, 130, 130,
...
bool CGame::Start(int iArgumentCount, char* szArguments[])
{
    ...
    {
        strBuffer = g_pServerInterface->GetModManager()->GetAbsolutePath("GninEserver.conf");
...
void CGame::Packet_PlayerJoinData(CPlayerJoinDataPacket& Packet)
{
    ...
    if (szNick && szNick[0] != 0)
    {
...
#if GninESA_VERSION_TYPE < VERSION_TYPE_UNSTABLE
        ...
        if (sizeNick >= MIN_PLAYER_NICK_LENGTH && sizeNick <= MAX_PLAYER_NICK_LENGTH)
        {
            ...
            {
                // Correct version?
                if (Packet.GetNetVersion() == GninE_DM_NETCODE_VERSION)
                {
                    ...
                    if (bPasswordIsValid)
                    {
                        ...
                        if (!m_pMainConfig->GetJoinFloodProtectionEnabled() || !m_FloodProtect.AddConnect(SString("%x", Packet.GetSourceIP())))
                        {
                            ...
                            pPlayer->SetGninEVersion(Packet.GetGninEVersion());
                ...
                else
                {
                    ...
                    ushort  usServerNetVersion = GninE_DM_NETCODE_VERSION;
                    ...
                    else if (GninESA_VERSION_BUILD == 0)
                    {
...
void CGame::PlayerCompleteConnect(CPlayer* pPlayer)
{
    ...
    Arguments.PushNumber(pPlayer->GetGninEVersion());
...
void CGame::HandleBackup()
{
    ...
    // Backup config files
    zipMaker.InsertFile(pModManager->GetAbsolutePath("GninEserver.conf"), PathJoin("config", "GninEserver.conf"));
