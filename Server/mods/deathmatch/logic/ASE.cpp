    ...
    : m_QueryDosProtect(5, 6000, 7000)            // Max of 5 queries per 6 seconds, then 7 second ignore
{
    ...
    m_strMtaAseVersion = GninE_DM_ASE_VERSION;
}
    ...
    for (uint s = 0; s < m_SocketList.size(); s++)
    {
        ...
        for (uint i = 0; i < 100; i++)
        {
            ...
            switch (szBuffer[0])
            {
                ...
                case 'v':
                {            // GninE Version (For further possibilities to quick ping, in case we do multiply master servers)
...
std::string ASE::QueryXfireLight()
{
    ...
    // version
    std::string temp = GninE_DM_ASE_VERSION;
...
std::string ASE::QueryLight()
{
    ...
    int              iMaxPlayers = m_pMainConfig->GetMaxPlayers();
    SString          strPlayerCount = SString("%d/%d", iJoinedPlayers, iMaxPlayers);
    SString          strBuildType = SString("%d", GninESA_VERSION_TYPE);
    SString          strBuildNumber = SString("%d", GninESA_VERSION_BUILD);
