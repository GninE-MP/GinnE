...
#define GninE_SERVER_CONF_TEMPLATE "GninEserver.conf.template"
...
bool CMainConfig::LoadExtended()
{
    ...
    RegisterCommand("ver", CConsoleCommands::Ver, false, "Get the GninE version");
    RegisterCommand("sver", CConsoleCommands::Ver, false, "Get the server GninE version");
...
{
    // Only GninEserver.conf is currently supported
    if (!g_pGame->IsUsingMtaServerConf())
        return false;

    SString strTemplateFilename = PathJoin(g_pServerInterface->GetServerModPath(), "GninEserver.conf.template");
    ...
    for (auto it = pRootNodeTemplate->ChildrenBegin(); it != pRootNodeTemplate->ChildrenEnd(); ++it)
    {
        ...
        if (!pNode)
        {
            CLogger::LogPrintf("Adding missing '%s' to GninEserver.conf\n", *strNodeName);
